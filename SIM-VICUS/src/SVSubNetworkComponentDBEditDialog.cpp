...
{
	m_toolButtonRemove = new QToolButton(m_ui->groupBoxTableView);
	m_toolButtonRemove->setObjectName(QString::fromUtf8("toolButtonRemove"));
	QIcon icon4;
	icon4.addFile(QString::fromUtf8(":/gfx/actions/24x24/minus.png"), QSize(), QIcon::Normal, QIcon::Off);
	m_toolButtonRemove->setIcon(icon4);
	m_toolButtonRemove->setIconSize(QSize(24, 24));
	m_ui->gridLayoutTableView->addWidget(m_toolButtonRemove, 3, 0, 1, 1);

	m_ui->label_3->setVisible(false);
	m_ui->frameProject->setVisible(false);

	connect(m_toolButtonRemove, &QToolButton::clicked, this, &SVSubNetworkComponentDBEditDialog::on_m_toolButtonRemove_clicked);
...
void SVSubNetworkComponentDBEditDialog::on_m_toolButtonRemove_clicked()
{
	m_toolButtonRemove->setFocus(); // move focus to trigger "leave" events in line edits
