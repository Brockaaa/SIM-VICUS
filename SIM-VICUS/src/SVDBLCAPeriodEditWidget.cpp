...
void SVDBLCAPeriodEditWidget::updateInput(int id) {
	...
	VICUS::LCAPeriod * bc = const_cast<VICUS::LCAPeriod *>(m_db->m_lcaPeriods[(unsigned int)id]);
	m_current = bc;
...
void SVDBLCAPeriodEditWidget::on_lineEditHeatTransferCoefficient_editingFinished() {
	Q_ASSERT(m_current != nullptr);
...
void SVDBLCAPeriodEditWidget::on_lineEditSolarAbsorptionCoefficient_editingFinished() {
	Q_ASSERT(m_current != nullptr);
...
void SVDBLCAPeriodEditWidget::on_lineEditLongWaveEmissivity_editingFinished() {
	Q_ASSERT(m_current != nullptr);
...
void SVDBLCAPeriodEditWidget::on_comboBoxHeatTransferCoeffModelType_currentIndexChanged(int index) {
	Q_ASSERT(m_current != nullptr);
	// update database but only if different from original
...
void SVDBLCAPeriodEditWidget::on_comboBoxLWModelType_currentIndexChanged(int index) {
	Q_ASSERT(m_current != nullptr);
	// update database but only if different from original
...
void SVDBLCAPeriodEditWidget::on_comboBoxSWModelType_currentIndexChanged(int index) {
	Q_ASSERT(m_current != nullptr);
	// update database but only if different from original
...
void SVDBLCAPeriodEditWidget::on_comboBoxConnectedZoneType_currentIndexChanged(int index) {
...
void SVDBLCAPeriodEditWidget::on_toolButtonSelectTemperatureSchedule_clicked() {
...
void SVDBLCAPeriodEditWidget::on_toolButtonRemoveTemperatureSchedule_clicked() {
...
void SVDBLCAPeriodEditWidget::on_lineEditZoneConstTemperature_editingFinishedSuccessfully() {
