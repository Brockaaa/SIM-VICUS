    ...
    ui(new Ui::SVSubNetworkEditDialog)
{
    ...
    ui->tbox->addPage(tr("Pipes"), tables[0]);
    ui->tbox->addPage(tr("Pumps"), tables[1]);
    ui->tbox->addPage(tr("Heatpumps"), tables[2]);
    ui->tbox->addPage(tr("Other"), tables[3]);
...
void SVSubNetworkEditDialog::createToolBox(){
    ...
    if(tables.size() == 0){
        for(int i = 0; i < static_cast<int>(VICUS::NetComCategory::NUM_NC); i++){
            tables.push_back(new SVSubNetworkEditDialogTable(this));
            connect(tables[i], &SVSubNetworkEditDialogTable::itemSelectionChanged, this, &SVSubNetworkEditDialog::on_componentSelected);
        }
    } else {
    ...
    auto addElement = [&](VICUS::NetworkComponent::ModelType type, VICUS::NetComCategory category){
        ...
        do{
            if(networkComponents->second.m_modelType == type){
                tables[static_cast<int>(category)]->addElement(networkComponents->second);
            }
            networkComponents++;
    ...
    for(const auto& pair : VICUS::ModelTypeAttributes){
        ...
        case VICUS::Pipes:
            tables[static_cast<int>(VICUS::NetComCategory::Pipes)]->addElement(pair.first);
            addElement(pair.first, pair.second.category);
            break;
        case VICUS::Pumps:
            tables[static_cast<int>(VICUS::NetComCategory::Pumps)]->addElement(pair.first);
            addElement(pair.first, pair.second.category);
            break;
        case VICUS::Heatpumps:
            tables[static_cast<int>(VICUS::NetComCategory::Heatpumps)]->addElement(pair.first);
            addElement(pair.first, pair.second.category);
            break;
        case VICUS::Other:
            tables[static_cast<int>(VICUS::NetComCategory::Other)]->addElement(pair.first);
            addElement(pair.first, pair.second.category);
            break;
        default:
...
void SVSubNetworkEditDialog::setNetwork()
{
    ...
    if(m_subNetwork->m_graphicalNetwork.m_blocks.size() == 0){
        m_sceneManager->setNetwork(m_subNetwork->m_graphicalNetwork);
        ...
        for(VICUS::NetworkElement element : m_subNetwork->m_elements){
            ...
            if(element.m_componentId != VICUS::INVALID_ID){
                ...
                unsigned int newId = getNewComponentID();
                component.m_id = block->m_elementID = newId;
                component.m_builtIn = false;
            ...
            if(element.m_controlElementId != VICUS::INVALID_ID){
                ...
                unsigned int newId = getNewControllerID();
                controller.m_id = block->m_controllerID = newId;
                controller.m_builtIn = false;
                m_networkControllers.push_back(controller);
            }

            VICUS::NetworkComponent::ModelType type = m_networkComponents[block->m_elementID].m_modelType;
            ...
            block->m_properties["Pixmap"] = QPixmap(VICUS::ModelTypeAttributes[type].fileName).scaled(256,256);
    ...
    // if BMNetwork filled, create necessary graphical elements
        ...
        for(auto &block : m_subNetwork->m_graphicalNetwork.m_blocks){
            ...
            if(ok){
                ...
                block.m_displayName = element.m_displayName;
                block.m_elementID = element.m_componentId;
                block.m_controllerID = element.m_controlElementId;
                block.m_size = QSizeF(VICUS::BLOCK_WIDTH, VICUS::BLOCK_HEIGHT);
                block.m_properties["ShowPixmap"] = true;
                block.m_properties["Pixmap"] = QPixmap(VICUS::ModelTypeAttributes[m_networkComponents[element.m_componentId].m_modelType].fileName).scaled(256,256);
                ...
                if(block.m_name == VICUS::ENTRANCE_NAME){
                ...
                else if(block.m_name == VICUS::EXIT_NAME){
                    block.m_size = QSizeF(VICUS::ENTRANCEEXITBLOCK_WIDTH, VICUS::ENTRANCEEXITBLOCK_HEIGHT);
                    block.m_displayName.clear();
                ...
                else if(block.m_name.contains(VICUS::CONNECTOR_NAME)){
                    block.m_displayName = "";
    ...
    for(auto& block : m_sceneManager->network().m_blocks){
        if(block.m_controllerID != VICUS::INVALID_ID){
            VICUS::BMBlock *unConstBlock = const_cast<VICUS::BMBlock*>(&block);
            m_sceneManager->setControllerID(unConstBlock, unConstBlock->m_controllerID,
                                            VICUS::KeywordListQt::Keyword("NetworkController::ControlledProperty", static_cast<int>(m_networkControllers[getControllerIndex(unConstBlock->m_controllerID)].m_controlledProperty)));
        }
    }
...
void SVSubNetworkEditDialog::on_buttonBox_accepted()
{
    ...
    for(unsigned int i = 0; i < m_subNetwork->m_components.size(); i++){
        bool toDelete = true;
        for(auto& block : m_subNetwork->m_graphicalNetwork.m_blocks){
            if(m_subNetwork->m_components[i].m_id == block.m_elementID){
                toDelete = false;
                break;
            }
        }
    ...
    for(auto& block : m_subNetwork->m_graphicalNetwork.m_blocks){
        ...
        block.m_name.toInt(&ok);
        if(ok){
            VICUS::NetworkElement networkElement(block.m_name.toInt(), block.m_sockets[1].m_id, block.m_sockets[0].m_id, block.m_elementID);
...
void SVSubNetworkEditDialog::blockSelectedEvent()
{
    ...
    if(blocks.size() == 1){
        ...
        // if Block is a ModelType Block, fill data in the right widget
        ...
        if(ok){
            ...
            nandradModelType = VICUS::NetworkComponent::nandradNetworkComponentModelType(m_networkComponents[getComponentIndex(blockToDisplay->m_elementID)].m_modelType);
            ...
            if(availableCtrProps.size() > 0){
                            ...
                            if(blockToDisplay->m_elementID != VICUS::INVALID_ID){
                                ui->openControllerWidgetButton->setEnabled(true);
                            }
                            ui->openControllerWidgetButton->setEnabled(false);
            }

            if(getControllerIndex(blockToDisplay->m_controllerID) != VICUS::INVALID_ID){
                ui->controllerLineEdit->setText(QString::fromLatin1(VICUS::KeywordListQt::Keyword("NetworkController::ControlledProperty", static_cast<int>(m_networkControllers[getControllerIndex(blockToDisplay->m_controllerID)].m_controlledProperty))));
                ui->removeControllerButton->setEnabled(true);
            }
            else{
            ...
            int index = getComponentIndex(blockToDisplay->m_elementID);
            ui->networkComponentEditWidget->updateInput(index);

        } else if(blockToDisplay->m_name.contains(VICUS::CONNECTORBLOCK_NAME)){
            ui->controllerGroupBox->setEnabled(true);
            ui->controllerLineEdit->setDisabled(true);
...
void SVSubNetworkEditDialog::on_controllerDialog_accepted(VICUS::BMBlock *block, VICUS::NetworkController controller)
{
    if(block->m_controllerID == VICUS::INVALID_ID){
        ...
        m_networkControllers[getControllerIndex(block->m_controllerID)] = controller;
    }
    ...
    m_sceneManager->setControllerID(block, block->m_controllerID, VICUS::KeywordListQt::Keyword("NetworkController::ControlledProperty", static_cast<int>(controller.m_controlledProperty)));
    m_sceneManager->update();
}
...
void SVSubNetworkEditDialog::on_newBlockAdded(VICUS::BMBlock *block, unsigned int componentID){
    if(componentID == VICUS::INVALID_ID){
        ...
        m_networkComponents.back().m_modelType = VICUS::ModelTypeLookup(block->m_elementID);
        QString name = QString("New ") + VICUS::ModelTypeAttributes[m_networkComponents.back().m_modelType].typeName;
        m_networkComponents.back().m_displayName = IBK::MultiLanguageString(name.toStdString());
        block->m_elementID = newComponentID;
        ...
        do{
            if(networkComponents->second.m_id == (unsigned int)componentID){
                ...
                component.m_builtIn = false;
                m_networkComponents.push_back(component);
                block->m_elementID = getNewComponentID();
                m_networkComponents.back().m_id = block->m_elementID;
                break;
            }
            networkComponents++;
        }while(networkComponents != m_db->m_networkComponents.end());
...
unsigned int SVSubNetworkEditDialog::getComponentIndex(unsigned int componentID)
{
...
unsigned int SVSubNetworkEditDialog::getNewComponentID()
{
    unsigned int id = 0;
    for( unsigned int i = 0; i < m_networkComponents.size(); i++){
...
unsigned int SVSubNetworkEditDialog::getControllerIndex(unsigned int controllerID)
{
...
void SVSubNetworkEditDialog::on_removeBlockOrConnectorButton_clicked()
{
    if(m_sceneManager->selectedBlocks().size() > 0){
        VICUS::BMBlock *selectedBlock = const_cast<VICUS::BMBlock*>(m_sceneManager->selectedBlocks().takeFirst());
        if(selectedBlock->m_name.contains(VICUS::ENTRANCE_NAME) || selectedBlock->m_name.contains(VICUS::EXIT_NAME)) return;
        else if(selectedBlock->m_name.contains(VICUS::CONNECTORBLOCK_NAME)){
            m_sceneManager->removeConnectorBlock(selectedBlock);
            m_sceneManager->update();
            return;
        } else{
            if(selectedBlock->m_elementID != VICUS::INVALID_ID)
                m_networkComponents.erase(m_networkComponents.begin() + getComponentIndex(selectedBlock->m_elementID));
            if(selectedBlock->m_controllerID != VICUS::INVALID_ID)
                m_networkControllers.erase(m_networkControllers.begin() + getControllerIndex(selectedBlock->m_controllerID));
...
void SVSubNetworkEditDialog::on_openControllerWidgetButton_clicked()
{
    ...
    if(selectedBlock->m_controllerID == VICUS::INVALID_ID){
        ...
        m_controllerEditWidget->setup(selectedBlock, *controller, &(m_networkComponents[getComponentIndex(selectedBlock->m_elementID)]));
    } else {
        m_controllerEditWidget->setup(selectedBlock, (m_networkControllers[getControllerIndex(selectedBlock->m_controllerID)]), &(m_networkComponents[getComponentIndex(selectedBlock->m_elementID)]));
    }
...
void SVSubNetworkEditDialog::on_copyBlockButton_clicked()
{
    ...
    VICUS::BMBlock *selectedBlock = const_cast<VICUS::BMBlock*>(m_sceneManager->selectedBlocks().takeFirst());
    Q_ASSERT(selectedBlock != nullptr);
    if(selectedBlock->m_name.contains(VICUS::ENTRANCE_NAME) || selectedBlock->m_name.contains(VICUS::EXIT_NAME) || selectedBlock->m_name.contains(VICUS::CONNECTORBLOCK_NAME)) return;
    else{
        VICUS::BMBlock newBlock = *selectedBlock;
        newBlock.m_pos += QPointF(2*VICUS::BLOCK_WIDTH, 0);
        if(selectedBlock->m_controllerID != VICUS::INVALID_ID){
            VICUS::NetworkController controller = m_networkControllers[getControllerIndex(selectedBlock->m_controllerID)];
            newBlock.m_controllerID = controller.m_id = getNewControllerID();
            controller.m_builtIn = false;
            m_networkControllers.push_back(controller);
        }

        VICUS::NetworkComponent component = m_networkComponents[getComponentIndex(selectedBlock->m_elementID)];
        newBlock.m_elementID = component.m_id = getNewComponentID();
        m_networkComponents.push_back(component);
            ...
            for(auto &blockItr : m_sceneManager->network().m_blocks){
                if(blockItr.m_controllerID == newBlock.m_controllerID){
                                m_sceneManager->setControllerID(const_cast<VICUS::BMBlock*>(&blockItr), newBlock.m_controllerID,
                                                                VICUS::KeywordListQt::Keyword("NetworkController::ControlledProperty", static_cast<int>(m_networkControllers[getControllerIndex(newBlock.m_controllerID)].m_controlledProperty)));
                }
            }
        }
        m_sceneManager->update();
...
void SVSubNetworkEditDialog::on_addToDBButton_clicked()
{
    ...
    qDebug() << m_db->m_networkComponents.add(m_networkComponents[getComponentIndex(selectedBlock->m_elementID)]) << "new ID";
    createToolBox();
}
