...
void Land::readXML(const TiXmlElement * element) {
	FUNCID(Land::readXML);
	...
	try {
		...
		throw IBK::Exception( ex, IBK::FormatString("Error reading 'Land' element."), FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'Land' element.").arg(ex2.what()), FUNC_ID);
	}
}

TiXmlElement * Land::writeXML(TiXmlElement * parent) const {
	TiXmlElement * e = new TiXmlElement("Land");
