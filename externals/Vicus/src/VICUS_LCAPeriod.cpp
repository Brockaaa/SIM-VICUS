...
bool LCAPeriod::isValid(const Database<Schedule> & scheduleDB) const {
...
bool LCAPeriod::hasSetpointTemperatureForZone() const {
...
QString LCAPeriod::htmlDescription(const VICUS::Database<Schedule> & scheduleDB) const {
...
AbstractDBElement::ComparisonResult LCAPeriod::equal(const AbstractDBElement *other) const {
	const LCAPeriod * otherBC = dynamic_cast<const LCAPeriod*>(other);
	if (otherBC == nullptr)
		return Different;
