/*	The NANDRAD data model library.

	Copyright (c) 2012-today, Institut für Bauklimatik, TU Dresden, Germany

	Primary authors:
	  Andreas Nicolai  <andreas.nicolai -[at]- tu-dresden.de>
	  Anne Paepcke     <anne.paepcke -[at]- tu-dresden.de>

	This library is part of SIM-VICUS (https://github.com/ghorwin/SIM-VICUS)

	This library is free software; you can redistribute it and/or
	modify it under the terms of the GNU Lesser General Public
	License as published by the Free Software Foundation; either
	version 3 of the License, or (at your option) any later version.

	This library is distributed in the hope that it will be useful,
	but WITHOUT ANY WARRANTY; without even the implied warranty of
	MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
	Lesser General Public License for more details.
*/

#include "VICUS_KeywordList.h"

#include <map>
#include <limits>
#include <iostream>

#include <IBK_FormatString.h>
#include <IBK_Exception.h>


namespace VICUS {
	/*! Holds the string to print as error when an invalid keyword is encountered. */
	const char * const INVALID_KEYWORD_INDEX_STRING = "KEYWORD_ERROR_STRING: Invalid type index";

	/*! Holds a list of all enum types/categories. */
	const char * const ENUM_TYPES[86] = {
		"AcousticBuildingTemplate::AcousticBuildingType",
		"AcousticComponent::para_t",
		"AcousticComponent::SizeMethod",
		"AcousticReferenceComponent::RequirementType",
		"AcousticReferenceComponent::ComponentType",
		"AcousticSoundAbsorptionPartition::para_t",
		"AcousticSoundProtectionTemplate::AcousticBuildingType",
		"AcousticTemplate::splinePara_t",
		"Component::ComponentType",
		"Construction::UsageType",
		"Construction::InsulationKind",
		"Construction::MaterialKind",
		"Construction::para_t",
		"EpdCategorySet::Category",
		"EpdDataset::Type",
		"EpdDataset::Category",
		"EpdModuleDataset::para_t",
		"EpdModuleDataset::Module",
		"Infiltration::para_t",
		"Infiltration::AirChangeType",
		"InterfaceHeatConduction::modelType_t",
		"InterfaceHeatConduction::OtherZoneType",
		"InterfaceHeatConduction::para_t",
		"InternalLoad::para_t",
		"InternalLoad::Category",
		"InternalLoad::PersonCountMethod",
		"InternalLoad::PowerMethod",
		"KeywordList::MyParameters",
		"LcaSettings::para_t",
		"LcaSettings::Module",
		"LcaSettings::LcaCategory",
		"LcaSettings::UsageType",
		"LcaSettings::CalculationMode",
		"LcaSettings::CertificationSytem",
		"LccSettings::para_t",
		"LccSettings::intPara_t",
		"Material::para_t",
		"Material::Category",
		"MaterialLayer::para_t",
		"Network::PipeModel",
		"Network::ModelType",
		"Network::NetworkType",
		"Network::para_t",
		"NetworkBuriedPipeProperties::SoilType",
		"NetworkBuriedPipeProperties::para_t",
		"NetworkComponent::ModelType",
		"NetworkComponent::para_t",
		"NetworkComponent::intPara_t",
		"NetworkComponent::ComponentCategory",
		"NetworkController::ModelType",
		"NetworkController::ControlledProperty",
		"NetworkController::ControllerType",
		"NetworkController::para_t",
		"NetworkController::References",
		"NetworkFluid::para_t",
		"NetworkHeatExchange::ModelType",
		"NetworkHeatExchange::para_t",
		"NetworkHeatExchange::BuildingType",
		"NetworkHeatExchange::AmbientTemperatureType",
		"NetworkNode::NodeType",
		"NetworkPipe::para_t",
		"OutputDefinition::timeType_t",
		"Outputs::flag_t",
		"Room::para_t",
		"SubSurfaceComponent::SubSurfaceComponentType",
		"SubSurfaceComponent::para_t",
		"SupplySystem::SupplyType",
		"SupplySystem::para_t",
		"SurfaceHeating::para_t",
		"SurfaceHeating::Type",
		"VentilationNatural::para_t",
		"Window::Method",
		"Window::para_t",
		"WindowDivider::para_t",
		"WindowFrame::para_t",
		"WindowGlazingSystem::modelType_t",
		"WindowGlazingSystem::para_t",
		"WindowGlazingSystem::splinePara_t",
		"ZoneControlNaturalVentilation::para_t",
		"ZoneControlShading::para_t",
		"ZoneControlShading::Category",
		"ZoneControlThermostat::para_t",
		"ZoneControlThermostat::ControlValue",
		"ZoneControlThermostat::ControllerType",
		"ZoneIdealHeatingCooling::para_t",
		"ZoneTemplate::SubTemplateType"
	};

	/*! Converts a category string to respective enumeration value. */
	int enum2index(const std::string & enumtype) {
		for (int i=0; i<86; ++i) {
			if (enumtype == ENUM_TYPES[i]) return i;
		}
		//std::cerr << "Unknown enumeration type '" << enumtype<< "'." << std::endl;
		return -1;
	}
	

	/*! Returns a keyword string for a given category (typenum) and type number t. */
	const char * theKeyword(int typenum, int t) {
		switch (typenum) {
			// AcousticBuildingTemplate::AcousticBuildingType
			case 0 :
			switch (t) {
				case 0 : return "MultiFamilyHouse";
				case 1 : return "Hotel";
				case 2 : return "Hospital";
				case 3 : return "School";
				case 4 : return "SingleFamilyHouse";
				case 5 : return "Office";
			} break;
			// AcousticComponent::para_t
			case 1 :
			switch (t) {
				case 0 : return "Area";
				case 1 : return "Fraction";
			} break;
			// AcousticComponent::SizeMethod
			case 2 :
			switch (t) {
				case 0 : return "AreaTotal";
				case 1 : return "AreaFraction";
			} break;
			// AcousticReferenceComponent::RequirementType
			case 3 :
			switch (t) {
				case 0 : return "basic";
				case 1 : return "advanced";
			} break;
			// AcousticReferenceComponent::ComponentType
			case 4 :
			switch (t) {
				case 0 : return "Ceiling";
				case 1 : return "Wall";
				case 2 : return "Door";
				case 3 : return "Stairs";
			} break;
			// AcousticSoundAbsorptionPartition::para_t
			case 5 :
			switch (t) {
				case 0 : return "AreaFraction";
			} break;
			// AcousticSoundProtectionTemplate::AcousticBuildingType
			case 6 :
			switch (t) {
				case 0 : return "MultiFamilyHouse";
				case 1 : return "Hotel";
				case 2 : return "Hospital";
				case 3 : return "School";
				case 4 : return "SingleFamilyHouse";
				case 5 : return "Office";
			} break;
			// AcousticTemplate::splinePara_t
			case 7 :
			switch (t) {
				case 0 : return "MaxValue";
				case 1 : return "MinValue";
			} break;
			// Component::ComponentType
			case 8 :
			switch (t) {
				case 0 : return "OutsideWall";
				case 1 : return "OutsideWallToGround";
				case 2 : return "InsideWall";
				case 3 : return "FloorToCellar";
				case 4 : return "FloorToAir";
				case 5 : return "FloorToGround";
				case 6 : return "Ceiling";
				case 7 : return "SlopedRoof";
				case 8 : return "FlatRoof";
				case 9 : return "ColdRoof";
				case 10 : return "WarmRoof";
				case 11 : return "Miscellaneous";
			} break;
			// Construction::UsageType
			case 9 :
			switch (t) {
				case 0 : return "OutsideWall";
				case 1 : return "OutsideWallToGround";
				case 2 : return "InsideWall";
				case 3 : return "FloorToCellar";
				case 4 : return "FloorToGround";
				case 5 : return "Ceiling";
				case 6 : return "SlopedRoof";
				case 7 : return "FlatRoof";
				case 8 : return "---";
			} break;
			// Construction::InsulationKind
			case 10 :
			switch (t) {
				case 0 : return "NotInsulated";
				case 1 : return "InsideInsulation";
				case 2 : return "CoreInsulation";
				case 3 : return "OutsideInsulation";
				case 4 : return "---";
			} break;
			// Construction::MaterialKind
			case 11 :
			switch (t) {
				case 0 : return "BrickMasonry";
				case 1 : return "NaturalStoneMasonry";
				case 2 : return "Concrete";
				case 3 : return "Wood";
				case 4 : return "FrameWork";
				case 5 : return "Loam";
				case 6 : return "---";
			} break;
			// Construction::para_t
			case 12 :
			switch (t) {
				case 0 : return "ImpactSoundValue";
				case 1 : return "AirSoundResistanceValue";
			} break;
			// EpdCategorySet::Category
			case 13 :
			switch (t) {
				case 0 : return "IDCategoryA";
				case 1 : return "IDCategoryB";
				case 2 : return "IDCategoryC";
				case 3 : return "IDCategoryD";
			} break;
			// EpdDataset::Type
			case 14 :
			switch (t) {
				case 0 : return "Generic";
				case 1 : return "Specific";
				case 2 : return "Average";
				case 3 : return "Representative";
				case 4 : return "Template";
			} break;
			// EpdDataset::Category
			case 15 :
			switch (t) {
				case 0 : return "A";
				case 1 : return "B";
				case 2 : return "C";
				case 3 : return "D";
			} break;
			// EpdModuleDataset::para_t
			case 16 :
			switch (t) {
				case 0 : return "AreaDensity";
				case 1 : return "DryDensity";
				case 2 : return "GWP";
				case 3 : return "ODP";
				case 4 : return "POCP";
				case 5 : return "AP";
				case 6 : return "EP";
				case 7 : return "PENRT";
				case 8 : return "PERT";
			} break;
			// EpdModuleDataset::Module
			case 17 :
			switch (t) {
				case 0 : return "A1";
				case 1 : return "A2";
				case 2 : return "A3";
				case 3 : return "A4";
				case 4 : return "A5";
				case 5 : return "B1";
				case 6 : return "B2";
				case 7 : return "B3";
				case 8 : return "B4";
				case 9 : return "B5";
				case 10 : return "B6";
				case 11 : return "B7";
				case 12 : return "C1";
				case 13 : return "C2";
				case 14 : return "C3";
				case 15 : return "C4";
				case 16 : return "D";
			} break;
			// Infiltration::para_t
			case 18 :
			switch (t) {
				case 0 : return "AirChangeRate";
				case 1 : return "ShieldingCoefficient";
			} break;
			// Infiltration::AirChangeType
			case 19 :
			switch (t) {
				case 0 : return "normal";
				case 1 : return "n50";
			} break;
			// InterfaceHeatConduction::modelType_t
			case 20 :
			switch (t) {
				case 0 : return "Constant";
				case 1 : return "None";
			} break;
			// InterfaceHeatConduction::OtherZoneType
			case 21 :
			switch (t) {
				case 0 : return "Standard";
				case 1 : return "Constant";
				case 2 : return "Scheduled";
			} break;
			// InterfaceHeatConduction::para_t
			case 22 :
			switch (t) {
				case 0 : return "HeatTransferCoefficient";
				case 1 : return "ConstTemperature";
			} break;
			// InternalLoad::para_t
			case 23 :
			switch (t) {
				case 0 : return "PersonCount";
				case 1 : return "PersonPerArea";
				case 2 : return "AreaPerPerson";
				case 3 : return "Power";
				case 4 : return "PowerPerArea";
				case 5 : return "MoistureProductionRatePerArea";
				case 6 : return "ConvectiveHeatFactor";
				case 7 : return "LatentHeatFactor";
				case 8 : return "LossHeatFactor";
			} break;
			// InternalLoad::Category
			case 24 :
			switch (t) {
				case 0 : return "Person";
				case 1 : return "ElectricEquiment";
				case 2 : return "Lighting";
				case 3 : return "Other";
			} break;
			// InternalLoad::PersonCountMethod
			case 25 :
			switch (t) {
				case 0 : return "PersonPerArea";
				case 1 : return "AreaPerPerson";
				case 2 : return "PersonCount";
			} break;
			// InternalLoad::PowerMethod
			case 26 :
			switch (t) {
				case 0 : return "PowerPerArea";
				case 1 : return "Power";
			} break;
			// KeywordList::MyParameters
			case 27 :
			switch (t) {
				case 0 : return "Temperature";
				case 1 : return "Mass";
			} break;
			// LcaSettings::para_t
			case 28 :
			switch (t) {
				case 0 : return "TimePeriod";
				case 1 : return "FactorSimpleMode";
				case 2 : return "NetUsageArea";
			} break;
			// LcaSettings::Module
			case 29 :
			switch (t) {
				case 0 : return "A1";
				case 1 : return "A2";
				case 2 : return "A3";
				case 3 : return "A4";
				case 4 : return "A5";
				case 5 : return "B1";
				case 6 : return "B2";
				case 7 : return "B3";
				case 8 : return "B4";
				case 9 : return "B5";
				case 10 : return "B6";
				case 11 : return "B7";
				case 12 : return "C1";
				case 13 : return "C2";
				case 14 : return "C3";
				case 15 : return "C4";
				case 16 : return "D";
			} break;
			// LcaSettings::LcaCategory
			case 30 :
			switch (t) {
				case 0 : return "A";
				case 1 : return "B";
				case 2 : return "C";
				case 3 : return "D";
			} break;
			// LcaSettings::UsageType
			case 31 :
			switch (t) {
				case 0 : return "Gas";
				case 1 : return "Electricity";
				case 2 : return "Coal";
			} break;
			// LcaSettings::CalculationMode
			case 32 :
			switch (t) {
				case 0 : return "Simple";
				case 1 : return "Detailed";
			} break;
			// LcaSettings::CertificationSytem
			case 33 :
			switch (t) {
				case 0 : return "BNB";
			} break;
			// LccSettings::para_t
			case 34 :
			switch (t) {
				case 0 : return "PriceIncreaseGeneral";
				case 1 : return "PriceIncreaseEnergy";
				case 2 : return "DiscountingInterestRate";
				case 3 : return "CoalConsumption";
				case 4 : return "GasConsumption";
				case 5 : return "ElectricityConsumption";
			} break;
			// LccSettings::intPara_t
			case 35 :
			switch (t) {
				case 0 : return "CoalPrice";
				case 1 : return "GasPrice";
				case 2 : return "ElectricityPrice";
			} break;
			// Material::para_t
			case 36 :
			switch (t) {
				case 0 : return "Density";
				case 1 : return "HeatCapacity";
				case 2 : return "Conductivity";
				case 3 : return "Mu";
				case 4 : return "W80";
				case 5 : return "Wsat";
			} break;
			// Material::Category
			case 37 :
			switch (t) {
				case 0 : return "Coating";
				case 1 : return "Plaster";
				case 2 : return "Bricks";
				case 3 : return "NaturalStones";
				case 4 : return "Cementitious";
				case 5 : return "Insulations";
				case 6 : return "BuildingBoards";
				case 7 : return "Woodbased";
				case 8 : return "NaturalMaterials";
				case 9 : return "Soils";
				case 10 : return "CladdingSystems";
				case 11 : return "Foils";
				case 12 : return "Miscellaneous";
			} break;
			// MaterialLayer::para_t
			case 38 :
			switch (t) {
				case 0 : return "Thickness";
				case 1 : return "Lifetime";
			} break;
			// Network::PipeModel
			case 39 :
			switch (t) {
				case 0 : return "SimplePipe";
				case 1 : return "DynamicPipe";
			} break;
			// Network::ModelType
			case 40 :
			switch (t) {
				case 0 : return "HydraulicNetwork";
				case 1 : return "ThermalHydraulicNetwork";
			} break;
			// Network::NetworkType
			case 41 :
			switch (t) {
				case 0 : return "SinglePipe";
				case 1 : return "DoublePipe";
			} break;
			// Network::para_t
			case 42 :
			switch (t) {
				case 0 : return "TemperatureSetpoint";
				case 1 : return "TemperatureDifference";
				case 2 : return "MaxPressureLoss";
				case 3 : return "ReferencePressure";
				case 4 : return "DefaultFluidTemperature";
				case 5 : return "InitialFluidTemperature";
				case 6 : return "MaxPipeDiscretization";
			} break;
			// NetworkBuriedPipeProperties::SoilType
			case 43 :
			switch (t) {
				case 0 : return "Sand";
				case 1 : return "Loam";
				case 2 : return "Silt";
			} break;
			// NetworkBuriedPipeProperties::para_t
			case 44 :
			switch (t) {
				case 0 : return "PipeSpacing";
				case 1 : return "PipeDepth";
			} break;
			// NetworkComponent::ModelType
			case 45 :
			switch (t) {
				case 0 : return "SimplePipe";
				case 1 : return "DynamicPipe";
				case 2 : return "ConstantPressurePump";
				case 3 : return "ConstantMassFluxPump";
				case 4 : return "ControlledPump";
				case 5 : return "VariablePressurePump";
				case 6 : return "HeatExchanger";
				case 7 : return "HeatPumpVariableIdealCarnotSourceSide";
				case 8 : return "HeatPumpVariableIdealCarnotSupplySide";
				case 9 : return "HeatPumpVariableSourceSide";
				case 10 : return "HeatPumpOnOffSourceSide";
				case 11 : return "ControlledValve";
				case 12 : return "ConstantPressureLossValve";
				case 13 : return "IdealHeaterCooler";
				case 14 : return "PressureLossElement";
			} break;
			// NetworkComponent::para_t
			case 46 :
			switch (t) {
				case 0 : return "HydraulicDiameter";
				case 1 : return "PressureLossCoefficient";
				case 2 : return "PressureHead";
				case 3 : return "MassFlux";
				case 4 : return "PumpMaximumEfficiency";
				case 5 : return "FractionOfMotorInefficienciesToFluidStream";
				case 6 : return "MaximumPressureHead";
				case 7 : return "PumpMaximumElectricalPower";
				case 8 : return "DesignPressureHead";
				case 9 : return "DesignMassFlux";
				case 10 : return "PressureHeadReductionFactor";
				case 11 : return "Volume";
				case 12 : return "PipeMaxDiscretizationWidth";
				case 13 : return "CarnotEfficiency";
				case 14 : return "MaximumHeatingPower";
				case 15 : return "MaximumCoolingPower";
				case 16 : return "PressureLoss";
				case 17 : return "MinimumOutletTemperature";
				case 18 : return "HeatingPowerB0W35";
				case 19 : return "HeatingBufferSupplyTemperature";
				case 20 : return "HeatingBufferReturnTemperature";
				case 21 : return "DHWBufferSupplyTemperature";
				case 22 : return "DHWBufferReturnTemperature";
				case 23 : return "HeatingBufferVolume";
				case 24 : return "DHWBufferVolume";
				case 25 : return "PipeLength";
			} break;
			// NetworkComponent::intPara_t
			case 47 :
			switch (t) {
				case 0 : return "NumberParallelPipes";
				case 1 : return "NumberParallelElements";
			} break;
			// NetworkComponent::ComponentCategory
			case 48 :
			switch (t) {
				case 0 : return "Pipes";
				case 1 : return "Pumps";
				case 2 : return "Heatpumps";
				case 3 : return "Other";
			} break;
			// NetworkController::ModelType
			case 49 :
			switch (t) {
				case 0 : return "Constant";
				case 1 : return "Scheduled";
			} break;
			// NetworkController::ControlledProperty
			case 50 :
			switch (t) {
				case 0 : return "TemperatureDifference";
				case 1 : return "TemperatureDifferenceOfFollowingElement";
				case 2 : return "ThermostatValue";
				case 3 : return "MassFlux";
				case 4 : return "PumpOperation";
				case 5 : return "PressureDifferenceWorstpoint";
			} break;
			// NetworkController::ControllerType
			case 51 :
			switch (t) {
				case 0 : return "PController";
				case 1 : return "PIController";
				case 2 : return "PIDController";
				case 3 : return "OnOffController";
			} break;
			// NetworkController::para_t
			case 52 :
			switch (t) {
				case 0 : return "Kp";
				case 1 : return "Ki";
				case 2 : return "Kd";
				case 3 : return "TemperatureDifferenceSetpoint";
				case 4 : return "MassFluxSetpoint";
				case 5 : return "HeatLossOfFollowingElementThreshold";
				case 6 : return "RelControllerErrorForIntegratorReset";
				case 7 : return "PressureDifferenceSetpoint";
			} break;
			// NetworkController::References
			case 53 :
			switch (t) {
				case 0 : return "ThermostatZone";
				case 1 : return "Schedule";
			} break;
			// NetworkFluid::para_t
			case 54 :
			switch (t) {
				case 0 : return "Density";
				case 1 : return "HeatCapacity";
				case 2 : return "Conductivity";
			} break;
			// NetworkHeatExchange::ModelType
			case 55 :
			switch (t) {
				case 0 : return "TemperatureConstant";
				case 1 : return "TemperatureSpline";
				case 2 : return "TemperatureConstantEvaporator";
				case 3 : return "TemperatureSplineEvaporator";
				case 4 : return "TemperatureZone";
				case 5 : return "TemperatureConstructionLayer";
				case 6 : return "HeatLossConstant";
				case 7 : return "HeatLossSpline";
				case 8 : return "HeatLossConstantCondenser";
				case 9 : return "HeatLossSplineCondenser";
				case 10 : return "HeatingDemandSpaceHeating";
			} break;
			// NetworkHeatExchange::para_t
			case 56 :
			switch (t) {
				case 0 : return "Temperature";
				case 1 : return "HeatLoss";
				case 2 : return "ExternalHeatTransferCoefficient";
				case 3 : return "FloorArea";
				case 4 : return "MaximumHeatingLoad";
				case 5 : return "HeatingEnergyDemand";
				case 6 : return "MaximumCoolingLoad";
				case 7 : return "CoolingEnergyDemand";
				case 8 : return "DomesticHotWaterDemand";
				case 9 : return "HeatingSupplyTemperature";
				case 10 : return "DomesticHotWaterSupplyTemperature";
			} break;
			// NetworkHeatExchange::BuildingType
			case 57 :
			switch (t) {
				case 0 : return "ResidentialBuildingSingleFamily";
				case 1 : return "ResidentialBuildingMultiFamily";
				case 2 : return "ResidentialBuildingLarge";
				case 3 : return "OfficeBuilding";
				case 4 : return "UserDefineBuilding";
			} break;
			// NetworkHeatExchange::AmbientTemperatureType
			case 58 :
			switch (t) {
				case 0 : return "UndisturbedSoilTemperature";
				case 1 : return "BoreholeHeatExchanger";
				case 2 : return "UserDefined";
			} break;
			// NetworkNode::NodeType
			case 59 :
			switch (t) {
				case 0 : return "SubStation";
				case 1 : return "Mixer";
				case 2 : return "Source";
			} break;
			// NetworkPipe::para_t
			case 60 :
			switch (t) {
				case 0 : return "DiameterOutside";
				case 1 : return "ThicknessWall";
				case 2 : return "RoughnessWall";
				case 3 : return "ThermalConductivityWall";
				case 4 : return "HeatCapacityWall";
				case 5 : return "DensityWall";
				case 6 : return "ThicknessInsulation";
				case 7 : return "ThermalConductivityInsulation";
				case 8 : return "ThicknessOuterLayer";
			} break;
			// OutputDefinition::timeType_t
			case 61 :
			switch (t) {
				case 0 : return "None";
				case 1 : return "Mean";
				case 2 : return "Integral";
			} break;
			// Outputs::flag_t
			case 62 :
			switch (t) {
				case 0 : return "BinaryFormat";
				case 1 : return "CreateDefaultZoneOutputs";
				case 2 : return "CreateDefaultNetworkOutputs";
				case 3 : return "CreateDefaultNetworkSummationModels";
			} break;
			// Room::para_t
			case 63 :
			switch (t) {
				case 0 : return "Area";
				case 1 : return "Volume";
				case 2 : return "HeatCapacity";
			} break;
			// SubSurfaceComponent::SubSurfaceComponentType
			case 64 :
			switch (t) {
				case 0 : return "Window";
				case 1 : return "Door";
				case 2 : return "Miscellaneous";
			} break;
			// SubSurfaceComponent::para_t
			case 65 :
			switch (t) {
				case 0 : return "ReductionFactor";
			} break;
			// SupplySystem::SupplyType
			case 66 :
			switch (t) {
				case 0 : return "StandAlone";
				case 1 : return "SubNetwork";
				case 2 : return "DatabaseFMU";
				case 3 : return "UserDefinedFMU";
			} break;
			// SupplySystem::para_t
			case 67 :
			switch (t) {
				case 0 : return "MaximumMassFlux";
				case 1 : return "SupplyTemperature";
				case 2 : return "MaximumMassFluxFMU";
				case 3 : return "HeatingPowerFMU";
			} break;
			// SurfaceHeating::para_t
			case 68 :
			switch (t) {
				case 0 : return "HeatingLimit";
				case 1 : return "CoolingLimit";
				case 2 : return "PipeSpacing";
				case 3 : return "MaxFluidVelocity";
				case 4 : return "TemperatureDifferenceSupplyReturn";
			} break;
			// SurfaceHeating::Type
			case 69 :
			switch (t) {
				case 0 : return "Ideal";
				case 1 : return "PipeRegister";
			} break;
			// VentilationNatural::para_t
			case 70 :
			switch (t) {
				case 0 : return "AirChangeRate";
			} break;
			// Window::Method
			case 71 :
			switch (t) {
				case 0 : return "None";
				case 1 : return "Fraction";
				case 2 : return "ConstantWidth";
			} break;
			// Window::para_t
			case 72 :
			switch (t) {
				case 0 : return "FrameWidth";
				case 1 : return "FrameFraction";
				case 2 : return "DividerWidth";
				case 3 : return "DividerFraction";
			} break;
			// WindowDivider::para_t
			case 73 :
			switch (t) {
				case 0 : return "Thickness";
			} break;
			// WindowFrame::para_t
			case 74 :
			switch (t) {
				case 0 : return "Thickness";
			} break;
			// WindowGlazingSystem::modelType_t
			case 75 :
			switch (t) {
				case 0 : return "Simple";
			} break;
			// WindowGlazingSystem::para_t
			case 76 :
			switch (t) {
				case 0 : return "ThermalTransmittance";
			} break;
			// WindowGlazingSystem::splinePara_t
			case 77 :
			switch (t) {
				case 0 : return "SHGC";
			} break;
			// ZoneControlNaturalVentilation::para_t
			case 78 :
			switch (t) {
				case 0 : return "MaximumAirChangeRateComfort";
				case 1 : return "TemperatureAirMax";
				case 2 : return "TemperatureAirMin";
				case 3 : return "WindSpeedMax";
			} break;
			// ZoneControlShading::para_t
			case 79 :
			switch (t) {
				case 0 : return "GlobalHorizontal";
				case 1 : return "GlobalNorth";
				case 2 : return "GlobalEast";
				case 3 : return "GlobalSouth";
				case 4 : return "GlobalWest";
				case 5 : return "DeadBand";
			} break;
			// ZoneControlShading::Category
			case 80 :
			switch (t) {
				case 0 : return "GlobalHorizontalSensor";
				case 1 : return "GlobalHorizontalAndVerticalSensors";
			} break;
			// ZoneControlThermostat::para_t
			case 81 :
			switch (t) {
				case 0 : return "Tolerance";
				case 1 : return "DeadBand";
			} break;
			// ZoneControlThermostat::ControlValue
			case 82 :
			switch (t) {
				case 0 : return "AirTemperature";
				case 1 : return "OperativeTemperature";
			} break;
			// ZoneControlThermostat::ControllerType
			case 83 :
			switch (t) {
				case 0 : return "Analog";
				case 1 : return "Digital";
			} break;
			// ZoneIdealHeatingCooling::para_t
			case 84 :
			switch (t) {
				case 0 : return "HeatingLimit";
				case 1 : return "CoolingLimit";
			} break;
			// ZoneTemplate::SubTemplateType
			case 85 :
			switch (t) {
				case 0 : return "IntLoadPerson";
				case 1 : return "IntLoadEquipment";
				case 2 : return "IntLoadLighting";
				case 3 : return "IntLoadOther";
				case 4 : return "ControlThermostat";
				case 5 : return "ControlShading";
				case 6 : return "ControlVentilationNatural";
				case 7 : return "Infiltration";
				case 8 : return "NaturalVentilation";
				case 9 : return "IdealHeatingCooling";
			} break;
		} // switch
		return INVALID_KEYWORD_INDEX_STRING;
	}

	/*! Returns all keywords including deprecated for a given category (typenum) and type number (t). */
	const char * allKeywords(int typenum, int t) {
		switch (typenum) {
			// AcousticBuildingTemplate::AcousticBuildingType
			case 0 :
			switch (t) {
				case 0 : return "MultiFamilyHouse";
				case 1 : return "Hotel";
				case 2 : return "Hospital";
				case 3 : return "School";
				case 4 : return "SingleFamilyHouse";
				case 5 : return "Office";
			} break;
			// AcousticComponent::para_t
			case 1 :
			switch (t) {
				case 0 : return "Area";
				case 1 : return "Fraction";
			} break;
			// AcousticComponent::SizeMethod
			case 2 :
			switch (t) {
				case 0 : return "AreaTotal";
				case 1 : return "AreaFraction";
			} break;
			// AcousticReferenceComponent::RequirementType
			case 3 :
			switch (t) {
				case 0 : return "basic";
				case 1 : return "advanced";
			} break;
			// AcousticReferenceComponent::ComponentType
			case 4 :
			switch (t) {
				case 0 : return "Ceiling";
				case 1 : return "Wall";
				case 2 : return "Door";
				case 3 : return "Stairs";
			} break;
			// AcousticSoundAbsorptionPartition::para_t
			case 5 :
			switch (t) {
				case 0 : return "AreaFraction";
			} break;
			// AcousticSoundProtectionTemplate::AcousticBuildingType
			case 6 :
			switch (t) {
				case 0 : return "MultiFamilyHouse";
				case 1 : return "Hotel";
				case 2 : return "Hospital";
				case 3 : return "School";
				case 4 : return "SingleFamilyHouse";
				case 5 : return "Office";
			} break;
			// AcousticTemplate::splinePara_t
			case 7 :
			switch (t) {
				case 0 : return "MaxValue";
				case 1 : return "MinValue";
			} break;
			// Component::ComponentType
			case 8 :
			switch (t) {
				case 0 : return "OutsideWall";
				case 1 : return "OutsideWallToGround";
				case 2 : return "InsideWall";
				case 3 : return "FloorToCellar";
				case 4 : return "FloorToAir";
				case 5 : return "FloorToGround";
				case 6 : return "Ceiling";
				case 7 : return "SlopedRoof";
				case 8 : return "FlatRoof";
				case 9 : return "ColdRoof";
				case 10 : return "WarmRoof";
				case 11 : return "Miscellaneous";
			} break;
			// Construction::UsageType
			case 9 :
			switch (t) {
				case 0 : return "OutsideWall";
				case 1 : return "OutsideWallToGround";
				case 2 : return "InsideWall";
				case 3 : return "FloorToCellar";
				case 4 : return "FloorToGround";
				case 5 : return "Ceiling";
				case 6 : return "SlopedRoof";
				case 7 : return "FlatRoof";
				case 8 : return "---";
			} break;
			// Construction::InsulationKind
			case 10 :
			switch (t) {
				case 0 : return "NotInsulated";
				case 1 : return "InsideInsulation";
				case 2 : return "CoreInsulation";
				case 3 : return "OutsideInsulation";
				case 4 : return "---";
			} break;
			// Construction::MaterialKind
			case 11 :
			switch (t) {
				case 0 : return "BrickMasonry";
				case 1 : return "NaturalStoneMasonry";
				case 2 : return "Concrete";
				case 3 : return "Wood";
				case 4 : return "FrameWork";
				case 5 : return "Loam";
				case 6 : return "---";
			} break;
			// Construction::para_t
			case 12 :
			switch (t) {
				case 0 : return "ImpactSoundValue";
				case 1 : return "AirSoundResistanceValue";
			} break;
			// EpdCategorySet::Category
			case 13 :
			switch (t) {
				case 0 : return "IDCategoryA";
				case 1 : return "IDCategoryB";
				case 2 : return "IDCategoryC";
				case 3 : return "IDCategoryD";
			} break;
			// EpdDataset::Type
			case 14 :
			switch (t) {
				case 0 : return "Generic";
				case 1 : return "Specific";
				case 2 : return "Average";
				case 3 : return "Representative";
				case 4 : return "Template";
			} break;
			// EpdDataset::Category
			case 15 :
			switch (t) {
				case 0 : return "A";
				case 1 : return "B";
				case 2 : return "C";
				case 3 : return "D";
			} break;
			// EpdModuleDataset::para_t
			case 16 :
			switch (t) {
				case 0 : return "AreaDensity";
				case 1 : return "DryDensity";
				case 2 : return "GWP";
				case 3 : return "ODP";
				case 4 : return "POCP";
				case 5 : return "AP";
				case 6 : return "EP";
				case 7 : return "PENRT";
				case 8 : return "PERT";
			} break;
			// EpdModuleDataset::Module
			case 17 :
			switch (t) {
				case 0 : return "A1";
				case 1 : return "A2";
				case 2 : return "A3";
				case 3 : return "A4";
				case 4 : return "A5";
				case 5 : return "B1";
				case 6 : return "B2";
				case 7 : return "B3";
				case 8 : return "B4";
				case 9 : return "B5";
				case 10 : return "B6";
				case 11 : return "B7";
				case 12 : return "C1";
				case 13 : return "C2";
				case 14 : return "C3";
				case 15 : return "C4";
				case 16 : return "D";
			} break;
			// Infiltration::para_t
			case 18 :
			switch (t) {
				case 0 : return "AirChangeRate";
				case 1 : return "ShieldingCoefficient";
			} break;
			// Infiltration::AirChangeType
			case 19 :
			switch (t) {
				case 0 : return "normal";
				case 1 : return "n50";
			} break;
			// InterfaceHeatConduction::modelType_t
			case 20 :
			switch (t) {
				case 0 : return "Constant";
				case 1 : return "None";
			} break;
			// InterfaceHeatConduction::OtherZoneType
			case 21 :
			switch (t) {
				case 0 : return "Standard";
				case 1 : return "Constant";
				case 2 : return "Scheduled";
			} break;
			// InterfaceHeatConduction::para_t
			case 22 :
			switch (t) {
				case 0 : return "HeatTransferCoefficient";
				case 1 : return "ConstTemperature";
			} break;
			// InternalLoad::para_t
			case 23 :
			switch (t) {
				case 0 : return "PersonCount";
				case 1 : return "PersonPerArea";
				case 2 : return "AreaPerPerson";
				case 3 : return "Power";
				case 4 : return "PowerPerArea";
				case 5 : return "MoistureProductionRatePerArea";
				case 6 : return "ConvectiveHeatFactor";
				case 7 : return "LatentHeatFactor";
				case 8 : return "LossHeatFactor";
			} break;
			// InternalLoad::Category
			case 24 :
			switch (t) {
				case 0 : return "Person";
				case 1 : return "ElectricEquiment";
				case 2 : return "Lighting";
				case 3 : return "Other";
			} break;
			// InternalLoad::PersonCountMethod
			case 25 :
			switch (t) {
				case 0 : return "PersonPerArea";
				case 1 : return "AreaPerPerson";
				case 2 : return "PersonCount";
			} break;
			// InternalLoad::PowerMethod
			case 26 :
			switch (t) {
				case 0 : return "PowerPerArea";
				case 1 : return "Power";
			} break;
			// KeywordList::MyParameters
			case 27 :
			switch (t) {
				case 0 : return "Temperature";
				case 1 : return "Mass";
			} break;
			// LcaSettings::para_t
			case 28 :
			switch (t) {
				case 0 : return "TimePeriod";
				case 1 : return "FactorSimpleMode";
				case 2 : return "NetUsageArea";
			} break;
			// LcaSettings::Module
			case 29 :
			switch (t) {
				case 0 : return "A1";
				case 1 : return "A2";
				case 2 : return "A3";
				case 3 : return "A4";
				case 4 : return "A5";
				case 5 : return "B1";
				case 6 : return "B2";
				case 7 : return "B3";
				case 8 : return "B4";
				case 9 : return "B5";
				case 10 : return "B6";
				case 11 : return "B7";
				case 12 : return "C1";
				case 13 : return "C2";
				case 14 : return "C3";
				case 15 : return "C4";
				case 16 : return "D";
			} break;
			// LcaSettings::LcaCategory
			case 30 :
			switch (t) {
				case 0 : return "A";
				case 1 : return "B";
				case 2 : return "C";
				case 3 : return "D";
			} break;
			// LcaSettings::UsageType
			case 31 :
			switch (t) {
				case 0 : return "Gas";
				case 1 : return "Electricity";
				case 2 : return "Coal";
			} break;
			// LcaSettings::CalculationMode
			case 32 :
			switch (t) {
				case 0 : return "Simple";
				case 1 : return "Detailed";
			} break;
			// LcaSettings::CertificationSytem
			case 33 :
			switch (t) {
				case 0 : return "BNB";
			} break;
			// LccSettings::para_t
			case 34 :
			switch (t) {
				case 0 : return "PriceIncreaseGeneral";
				case 1 : return "PriceIncreaseEnergy";
				case 2 : return "DiscountingInterestRate";
				case 3 : return "CoalConsumption";
				case 4 : return "GasConsumption";
				case 5 : return "ElectricityConsumption";
			} break;
			// LccSettings::intPara_t
			case 35 :
			switch (t) {
				case 0 : return "CoalPrice";
				case 1 : return "GasPrice";
				case 2 : return "ElectricityPrice";
			} break;
			// Material::para_t
			case 36 :
			switch (t) {
				case 0 : return "Density";
				case 1 : return "HeatCapacity";
				case 2 : return "Conductivity";
				case 3 : return "Mu";
				case 4 : return "W80";
				case 5 : return "Wsat";
			} break;
			// Material::Category
			case 37 :
			switch (t) {
				case 0 : return "Coating";
				case 1 : return "Plaster";
				case 2 : return "Bricks";
				case 3 : return "NaturalStones";
				case 4 : return "Cementitious";
				case 5 : return "Insulations";
				case 6 : return "BuildingBoards";
				case 7 : return "Woodbased";
				case 8 : return "NaturalMaterials";
				case 9 : return "Soils";
				case 10 : return "CladdingSystems";
				case 11 : return "Foils";
				case 12 : return "Miscellaneous";
			} break;
			// MaterialLayer::para_t
			case 38 :
			switch (t) {
				case 0 : return "Thickness";
				case 1 : return "Lifetime";
			} break;
			// Network::PipeModel
			case 39 :
			switch (t) {
				case 0 : return "SimplePipe";
				case 1 : return "DynamicPipe";
			} break;
			// Network::ModelType
			case 40 :
			switch (t) {
				case 0 : return "HydraulicNetwork";
				case 1 : return "ThermalHydraulicNetwork";
			} break;
			// Network::NetworkType
			case 41 :
			switch (t) {
				case 0 : return "SinglePipe";
				case 1 : return "DoublePipe";
			} break;
			// Network::para_t
			case 42 :
			switch (t) {
				case 0 : return "TemperatureSetpoint";
				case 1 : return "TemperatureDifference";
				case 2 : return "MaxPressureLoss";
				case 3 : return "ReferencePressure";
				case 4 : return "DefaultFluidTemperature";
				case 5 : return "InitialFluidTemperature";
				case 6 : return "MaxPipeDiscretization";
			} break;
			// NetworkBuriedPipeProperties::SoilType
			case 43 :
			switch (t) {
				case 0 : return "Sand";
				case 1 : return "Loam";
				case 2 : return "Silt";
			} break;
			// NetworkBuriedPipeProperties::para_t
			case 44 :
			switch (t) {
				case 0 : return "PipeSpacing";
				case 1 : return "PipeDepth";
			} break;
			// NetworkComponent::ModelType
			case 45 :
			switch (t) {
				case 0 : return "SimplePipe";
				case 1 : return "DynamicPipe";
				case 2 : return "ConstantPressurePump";
				case 3 : return "ConstantMassFluxPump";
				case 4 : return "ControlledPump";
				case 5 : return "VariablePressurePump";
				case 6 : return "HeatExchanger";
				case 7 : return "HeatPumpVariableIdealCarnotSourceSide";
				case 8 : return "HeatPumpVariableIdealCarnotSupplySide";
				case 9 : return "HeatPumpVariableSourceSide";
				case 10 : return "HeatPumpOnOffSourceSide";
				case 11 : return "ControlledValve";
				case 12 : return "ConstantPressureLossValve";
				case 13 : return "IdealHeaterCooler";
				case 14 : return "PressureLossElement";
			} break;
			// NetworkComponent::para_t
			case 46 :
			switch (t) {
				case 0 : return "HydraulicDiameter";
				case 1 : return "PressureLossCoefficient";
				case 2 : return "PressureHead";
				case 3 : return "MassFlux";
				case 4 : return "PumpMaximumEfficiency";
				case 5 : return "FractionOfMotorInefficienciesToFluidStream";
				case 6 : return "MaximumPressureHead";
				case 7 : return "PumpMaximumElectricalPower";
				case 8 : return "DesignPressureHead";
				case 9 : return "DesignMassFlux";
				case 10 : return "PressureHeadReductionFactor";
				case 11 : return "Volume";
				case 12 : return "PipeMaxDiscretizationWidth";
				case 13 : return "CarnotEfficiency";
				case 14 : return "MaximumHeatingPower";
				case 15 : return "MaximumCoolingPower";
				case 16 : return "PressureLoss";
				case 17 : return "MinimumOutletTemperature";
				case 18 : return "HeatingPowerB0W35";
				case 19 : return "HeatingBufferSupplyTemperature";
				case 20 : return "HeatingBufferReturnTemperature";
				case 21 : return "DHWBufferSupplyTemperature";
				case 22 : return "DHWBufferReturnTemperature";
				case 23 : return "HeatingBufferVolume";
				case 24 : return "DHWBufferVolume";
				case 25 : return "PipeLength";
			} break;
			// NetworkComponent::intPara_t
			case 47 :
			switch (t) {
				case 0 : return "NumberParallelPipes";
				case 1 : return "NumberParallelElements";
			} break;
			// NetworkComponent::ComponentCategory
			case 48 :
			switch (t) {
				case 0 : return "Pipes";
				case 1 : return "Pumps";
				case 2 : return "Heatpumps";
				case 3 : return "Other";
			} break;
			// NetworkController::ModelType
			case 49 :
			switch (t) {
				case 0 : return "Constant";
				case 1 : return "Scheduled";
			} break;
			// NetworkController::ControlledProperty
			case 50 :
			switch (t) {
				case 0 : return "TemperatureDifference";
				case 1 : return "TemperatureDifferenceOfFollowingElement";
				case 2 : return "ThermostatValue";
				case 3 : return "MassFlux";
				case 4 : return "PumpOperation";
				case 5 : return "PressureDifferenceWorstpoint";
			} break;
			// NetworkController::ControllerType
			case 51 :
			switch (t) {
				case 0 : return "PController";
				case 1 : return "PIController";
				case 2 : return "PIDController";
				case 3 : return "OnOffController";
			} break;
			// NetworkController::para_t
			case 52 :
			switch (t) {
				case 0 : return "Kp";
				case 1 : return "Ki";
				case 2 : return "Kd";
				case 3 : return "TemperatureDifferenceSetpoint";
				case 4 : return "MassFluxSetpoint";
				case 5 : return "HeatLossOfFollowingElementThreshold";
				case 6 : return "RelControllerErrorForIntegratorReset";
				case 7 : return "PressureDifferenceSetpoint";
			} break;
			// NetworkController::References
			case 53 :
			switch (t) {
				case 0 : return "ThermostatZone";
				case 1 : return "Schedule";
			} break;
			// NetworkFluid::para_t
			case 54 :
			switch (t) {
				case 0 : return "Density";
				case 1 : return "HeatCapacity";
				case 2 : return "Conductivity";
			} break;
			// NetworkHeatExchange::ModelType
			case 55 :
			switch (t) {
				case 0 : return "TemperatureConstant";
				case 1 : return "TemperatureSpline";
				case 2 : return "TemperatureConstantEvaporator";
				case 3 : return "TemperatureSplineEvaporator";
				case 4 : return "TemperatureZone";
				case 5 : return "TemperatureConstructionLayer";
				case 6 : return "HeatLossConstant";
				case 7 : return "HeatLossSpline";
				case 8 : return "HeatLossConstantCondenser";
				case 9 : return "HeatLossSplineCondenser";
				case 10 : return "HeatingDemandSpaceHeating";
			} break;
			// NetworkHeatExchange::para_t
			case 56 :
			switch (t) {
				case 0 : return "Temperature";
				case 1 : return "HeatLoss";
				case 2 : return "ExternalHeatTransferCoefficient";
				case 3 : return "FloorArea";
				case 4 : return "MaximumHeatingLoad";
				case 5 : return "HeatingEnergyDemand";
				case 6 : return "MaximumCoolingLoad";
				case 7 : return "CoolingEnergyDemand";
				case 8 : return "DomesticHotWaterDemand";
				case 9 : return "HeatingSupplyTemperature";
				case 10 : return "DomesticHotWaterSupplyTemperature";
			} break;
			// NetworkHeatExchange::BuildingType
			case 57 :
			switch (t) {
				case 0 : return "ResidentialBuildingSingleFamily";
				case 1 : return "ResidentialBuildingMultiFamily";
				case 2 : return "ResidentialBuildingLarge";
				case 3 : return "OfficeBuilding";
				case 4 : return "UserDefineBuilding";
			} break;
			// NetworkHeatExchange::AmbientTemperatureType
			case 58 :
			switch (t) {
				case 0 : return "UndisturbedSoilTemperature";
				case 1 : return "BoreholeHeatExchanger";
				case 2 : return "UserDefined";
			} break;
			// NetworkNode::NodeType
			case 59 :
			switch (t) {
				case 0 : return "SubStation";
				case 1 : return "Mixer";
				case 2 : return "Source";
			} break;
			// NetworkPipe::para_t
			case 60 :
			switch (t) {
				case 0 : return "DiameterOutside";
				case 1 : return "ThicknessWall";
				case 2 : return "RoughnessWall";
				case 3 : return "ThermalConductivityWall";
				case 4 : return "HeatCapacityWall";
				case 5 : return "DensityWall";
				case 6 : return "ThicknessInsulation";
				case 7 : return "ThermalConductivityInsulation";
				case 8 : return "ThicknessOuterLayer";
			} break;
			// OutputDefinition::timeType_t
			case 61 :
			switch (t) {
				case 0 : return "None";
				case 1 : return "Mean";
				case 2 : return "Integral";
			} break;
			// Outputs::flag_t
			case 62 :
			switch (t) {
				case 0 : return "BinaryFormat";
				case 1 : return "CreateDefaultZoneOutputs";
				case 2 : return "CreateDefaultNetworkOutputs";
				case 3 : return "CreateDefaultNetworkSummationModels";
			} break;
			// Room::para_t
			case 63 :
			switch (t) {
				case 0 : return "Area";
				case 1 : return "Volume";
				case 2 : return "HeatCapacity";
			} break;
			// SubSurfaceComponent::SubSurfaceComponentType
			case 64 :
			switch (t) {
				case 0 : return "Window";
				case 1 : return "Door";
				case 2 : return "Miscellaneous";
			} break;
			// SubSurfaceComponent::para_t
			case 65 :
			switch (t) {
				case 0 : return "ReductionFactor";
			} break;
			// SupplySystem::SupplyType
			case 66 :
			switch (t) {
				case 0 : return "StandAlone";
				case 1 : return "SubNetwork";
				case 2 : return "DatabaseFMU";
				case 3 : return "UserDefinedFMU";
			} break;
			// SupplySystem::para_t
			case 67 :
			switch (t) {
				case 0 : return "MaximumMassFlux";
				case 1 : return "SupplyTemperature";
				case 2 : return "MaximumMassFluxFMU";
				case 3 : return "HeatingPowerFMU";
			} break;
			// SurfaceHeating::para_t
			case 68 :
			switch (t) {
				case 0 : return "HeatingLimit";
				case 1 : return "CoolingLimit";
				case 2 : return "PipeSpacing";
				case 3 : return "MaxFluidVelocity";
				case 4 : return "TemperatureDifferenceSupplyReturn";
			} break;
			// SurfaceHeating::Type
			case 69 :
			switch (t) {
				case 0 : return "Ideal";
				case 1 : return "PipeRegister";
			} break;
			// VentilationNatural::para_t
			case 70 :
			switch (t) {
				case 0 : return "AirChangeRate";
			} break;
			// Window::Method
			case 71 :
			switch (t) {
				case 0 : return "None";
				case 1 : return "Fraction";
				case 2 : return "ConstantWidth";
			} break;
			// Window::para_t
			case 72 :
			switch (t) {
				case 0 : return "FrameWidth";
				case 1 : return "FrameFraction";
				case 2 : return "DividerWidth";
				case 3 : return "DividerFraction";
			} break;
			// WindowDivider::para_t
			case 73 :
			switch (t) {
				case 0 : return "Thickness";
			} break;
			// WindowFrame::para_t
			case 74 :
			switch (t) {
				case 0 : return "Thickness";
			} break;
			// WindowGlazingSystem::modelType_t
			case 75 :
			switch (t) {
				case 0 : return "Simple";
			} break;
			// WindowGlazingSystem::para_t
			case 76 :
			switch (t) {
				case 0 : return "ThermalTransmittance";
			} break;
			// WindowGlazingSystem::splinePara_t
			case 77 :
			switch (t) {
				case 0 : return "SHGC";
			} break;
			// ZoneControlNaturalVentilation::para_t
			case 78 :
			switch (t) {
				case 0 : return "MaximumAirChangeRateComfort";
				case 1 : return "TemperatureAirMax";
				case 2 : return "TemperatureAirMin";
				case 3 : return "WindSpeedMax";
			} break;
			// ZoneControlShading::para_t
			case 79 :
			switch (t) {
				case 0 : return "GlobalHorizontal";
				case 1 : return "GlobalNorth";
				case 2 : return "GlobalEast";
				case 3 : return "GlobalSouth";
				case 4 : return "GlobalWest";
				case 5 : return "DeadBand";
			} break;
			// ZoneControlShading::Category
			case 80 :
			switch (t) {
				case 0 : return "GlobalHorizontalSensor";
				case 1 : return "GlobalHorizontalAndVerticalSensors";
			} break;
			// ZoneControlThermostat::para_t
			case 81 :
			switch (t) {
				case 0 : return "Tolerance";
				case 1 : return "DeadBand";
			} break;
			// ZoneControlThermostat::ControlValue
			case 82 :
			switch (t) {
				case 0 : return "AirTemperature";
				case 1 : return "OperativeTemperature";
			} break;
			// ZoneControlThermostat::ControllerType
			case 83 :
			switch (t) {
				case 0 : return "Analog";
				case 1 : return "Digital";
			} break;
			// ZoneIdealHeatingCooling::para_t
			case 84 :
			switch (t) {
				case 0 : return "HeatingLimit";
				case 1 : return "CoolingLimit";
			} break;
			// ZoneTemplate::SubTemplateType
			case 85 :
			switch (t) {
				case 0 : return "IntLoadPerson";
				case 1 : return "IntLoadEquipment";
				case 2 : return "IntLoadLighting";
				case 3 : return "IntLoadOther";
				case 4 : return "ControlThermostat";
				case 5 : return "ControlShading";
				case 6 : return "ControlVentilationNatural";
				case 7 : return "Infiltration";
				case 8 : return "NaturalVentilation";
				case 9 : return "IdealHeatingCooling";
			} break;
		} // switch
		return INVALID_KEYWORD_INDEX_STRING;
	}

	const char * KeywordList::Description(const char * const enumtype, int t, bool * no_description) {
		if (no_description != nullptr)
			*no_description = false; // we are optimistic
		switch (enum2index(enumtype)) {
			// AcousticBuildingTemplate::AcousticBuildingType
			case 0 :
			switch (t) {
				case 0 : return "Wohngebäude und Gebäude mit Wohn- und Arbeitsbereichen";
				case 1 : return "Hotels und Beherbergungsstätten";
				case 2 : return "Krankenhäuser und Sanatorien";
				case 3 : return "Schulen und vergleichbaren Einrichtungen";
				case 4 : return "Einfamilien-Reihenhäusern und zwischen Doppelhäusern";
				case 5 : return "Büro";
			} break;
			// AcousticComponent::para_t
			case 1 :
			switch (t) {
				case 0 : return "Area";
				case 1 : return "Fraction";
			} break;
			// AcousticComponent::SizeMethod
			case 2 :
			switch (t) {
				case 0 : return "Total Area";
				case 1 : return "Fraction of Area";
			} break;
			// AcousticReferenceComponent::RequirementType
			case 3 :
			switch (t) {
				case 0 : return "Normal sound requirements";
				case 1 : return "Enhanced sound requirements";
			} break;
			// AcousticReferenceComponent::ComponentType
			case 4 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "Ceiling";
				case 1 : if (no_description != nullptr) *no_description = true; return "Wall";
				case 2 : if (no_description != nullptr) *no_description = true; return "Door";
				case 3 : if (no_description != nullptr) *no_description = true; return "Stairs";
			} break;
			// AcousticSoundAbsorptionPartition::para_t
			case 5 :
			switch (t) {
				case 0 : return "Fraction of surface area.";
			} break;
			// AcousticSoundProtectionTemplate::AcousticBuildingType
			case 6 :
			switch (t) {
				case 0 : return "Wohngebäude und Gebäude mit Wohn- und Arbeitsbereichen";
				case 1 : return "Hotels und Beherbergungsstätten";
				case 2 : return "Krankenhäuser und Sanatorien";
				case 3 : return "Schulen und vergleichbaren Einrichtungen";
				case 4 : return "Einfamilien-Reihenhäusern und zwischen Doppelhäusern";
				case 5 : return "Büro";
			} break;
			// AcousticTemplate::splinePara_t
			case 7 :
			switch (t) {
				case 0 : return "Max values for evaluation";
				case 1 : return "Max values for evaluation";
			} break;
			// Component::ComponentType
			case 8 :
			switch (t) {
				case 0 : return "Outside wall construction";
				case 1 : return "Outside wall construction in contact with ground";
				case 2 : return "Interior construction";
				case 3 : return "Floor to basement";
				case 4 : return "Floor in contact with air";
				case 5 : return "Floor in contact with ground";
				case 6 : return "Ceiling construction";
				case 7 : return "Sloped roof construction";
				case 8 : return "Flat roof construction";
				case 9 : return "Flat roof construction (to heated/insulated space)";
				case 10 : return "Flat roof construction (to cold/ventilated space)";
				case 11 : return "Some other component type";
			} break;
			// Construction::UsageType
			case 9 :
			switch (t) {
				case 0 : return "Outside wall construction";
				case 1 : return "Outside wall construction in contact with ground";
				case 2 : return "Interior construction";
				case 3 : return "Floor to basement";
				case 4 : return "Floor in contact with ground";
				case 5 : return "Ceiling construction";
				case 6 : return "Sloped roof construction";
				case 7 : return "Flat roof construction";
				case 8 : return "Miscellaneous";
			} break;
			// Construction::InsulationKind
			case 10 :
			switch (t) {
				case 0 : return "Not insulated";
				case 1 : return "Inside insulated";
				case 2 : return "Core insulation";
				case 3 : return "Outside insulated";
				case 4 : return "Not selected";
			} break;
			// Construction::MaterialKind
			case 11 :
			switch (t) {
				case 0 : return "Brick masonry";
				case 1 : return "Natural stones";
				case 2 : return "Concrete";
				case 3 : return "Wood";
				case 4 : return "Frame construction";
				case 5 : return "Loam";
				case 6 : return "Not selected";
			} break;
			// Construction::para_t
			case 12 :
			switch (t) {
				case 0 : return "Impact sound value";
				case 1 : return "Air sound resistance value";
			} break;
			// EpdCategorySet::Category
			case 13 :
			switch (t) {
				case 0 : return "ID of Catefory A";
				case 1 : return "ID of Catefory B";
				case 2 : return "ID of Catefory C";
				case 3 : return "ID of Catefory D";
			} break;
			// EpdDataset::Type
			case 14 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "Generic";
				case 1 : if (no_description != nullptr) *no_description = true; return "Specific";
				case 2 : if (no_description != nullptr) *no_description = true; return "Average";
				case 3 : if (no_description != nullptr) *no_description = true; return "Representative";
				case 4 : if (no_description != nullptr) *no_description = true; return "Template";
			} break;
			// EpdDataset::Category
			case 15 :
			switch (t) {
				case 0 : return "Production";
				case 1 : return "Usage";
				case 2 : return "Disposal";
				case 3 : return "Deposit";
			} break;
			// EpdModuleDataset::para_t
			case 16 :
			switch (t) {
				case 0 : return "mass of material in dependence of the area.";
				case 1 : return "Dry density of the material.";
				case 2 : return "Global Warming Potential.";
				case 3 : return "Depletion potential of the stratospheric ozone layer.";
				case 4 : return "Photochemical Ozone Creation Potential.";
				case 5 : return "Acidification potential.";
				case 6 : return "Eutrophication potential.";
				case 7 : return "Total use of non-renewable primary energy resources.";
				case 8 : return "Total use of renewable primary energy resources .";
			} break;
			// EpdModuleDataset::Module
			case 17 :
			switch (t) {
				case 0 : return "A1: Raw material procurement";
				case 1 : return "A2: Transport";
				case 2 : return "A3: Production";
				case 3 : return "A4: Transport";
				case 4 : return "A5: Construction / Installation";
				case 5 : return "B1: Usage";
				case 6 : return "B2: Maintenance";
				case 7 : return "B3: Maintenance";
				case 8 : return "B4: Replacement";
				case 9 : return "B5: Modernisation";
				case 10 : return "B6: Energy consumption in operation";
				case 11 : return "B7: Water consumption in operation";
				case 12 : return "C1: deconstruction / demolition";
				case 13 : return "C2: Transport";
				case 14 : return "C3: Waste recycling";
				case 15 : return "C4: Disposal";
				case 16 : return "D: Potential for reuse, recovery and recycling";
			} break;
			// Infiltration::para_t
			case 18 :
			switch (t) {
				case 0 : return "Air change rate";
				case 1 : return "Shielding coefficient for n50 value";
			} break;
			// Infiltration::AirChangeType
			case 19 :
			switch (t) {
				case 0 : return "normal";
				case 1 : return "n50";
			} break;
			// InterfaceHeatConduction::modelType_t
			case 20 :
			switch (t) {
				case 0 : return "Constant heat exchange coefficient";
				case 1 : return "No convective heat exchange";
			} break;
			// InterfaceHeatConduction::OtherZoneType
			case 21 :
			switch (t) {
				case 0 : return "Active zone or outside";
				case 1 : return "Zone/ground with constant temperature";
				case 2 : return "Zone/ground with scheduled temperature";
			} break;
			// InterfaceHeatConduction::para_t
			case 22 :
			switch (t) {
				case 0 : return "Convective heat transfer coefficient";
				case 1 : return "Constant temperature of other zone/ground";
			} break;
			// InternalLoad::para_t
			case 23 :
			switch (t) {
				case 0 : return "Person count";
				case 1 : return "Person per area";
				case 2 : return "Area per person";
				case 3 : return "Power";
				case 4 : return "Power per area";
				case 5 : return "Moisture production rate per area";
				case 6 : return "Convective heat factor";
				case 7 : return "Latent heat factor";
				case 8 : return "Loss heat factor";
			} break;
			// InternalLoad::Category
			case 24 :
			switch (t) {
				case 0 : return "Person";
				case 1 : return "ElectricEquiment";
				case 2 : return "Lighting";
				case 3 : return "Other";
			} break;
			// InternalLoad::PersonCountMethod
			case 25 :
			switch (t) {
				case 0 : return "Person per m2";
				case 1 : return "m2 per Person";
				case 2 : return "Person count";
			} break;
			// InternalLoad::PowerMethod
			case 26 :
			switch (t) {
				case 0 : return "Power per area";
				case 1 : return "Power";
			} break;
			// KeywordList::MyParameters
			case 27 :
			switch (t) {
				case 0 : return "Some temperatures";
				case 1 : return "Some mass";
			} break;
			// LcaSettings::para_t
			case 28 :
			switch (t) {
				case 0 : return "Time period for consideration [a].";
				case 1 : return "Calculation factor for simple mode calculation (BNB). [-]";
				case 2 : return "Net usage area [m2].";
			} break;
			// LcaSettings::Module
			case 29 :
			switch (t) {
				case 0 : return "Raw material procurement (A1)";
				case 1 : return "Transport (A2)";
				case 2 : return "Production (A3)";
				case 3 : return "Transport (A4)";
				case 4 : return "Construction / Installation (A5)";
				case 5 : return "Usage (B1)";
				case 6 : return "Maintenance (B2)";
				case 7 : return "Maintenance (B3)";
				case 8 : return "Replacement (B4)";
				case 9 : return "Modernisation (B5)";
				case 10 : return "Energy consumption in operation (B6)";
				case 11 : return "Water consumption in operation (B7)";
				case 12 : return "deconstruction / demolition (C1)";
				case 13 : return "Transport (C2)";
				case 14 : return "Waste recycling (C3)";
				case 15 : return "Disposal (C4)";
				case 16 : return "Potential for reuse, recovery and recycling (D)";
			} break;
			// LcaSettings::LcaCategory
			case 30 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "A";
				case 1 : if (no_description != nullptr) *no_description = true; return "B";
				case 2 : if (no_description != nullptr) *no_description = true; return "C";
				case 3 : if (no_description != nullptr) *no_description = true; return "D";
			} break;
			// LcaSettings::UsageType
			case 31 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "Gas";
				case 1 : if (no_description != nullptr) *no_description = true; return "Electricity";
				case 2 : if (no_description != nullptr) *no_description = true; return "Coal";
			} break;
			// LcaSettings::CalculationMode
			case 32 :
			switch (t) {
				case 0 : return "Use predefined certification system settings.";
				case 1 : return "Set detailed LCA Settings.";
			} break;
			// LcaSettings::CertificationSytem
			case 33 :
			switch (t) {
				case 0 : return "Bewertungssystem Nachhaltiges Bauen (BNB)";
			} break;
			// LccSettings::para_t
			case 34 :
			switch (t) {
				case 0 : return "Yearly general price increase [%].";
				case 1 : return "Yearly energy price increase [%].";
				case 2 : return "Interest rate for discounting [%].";
				case 3 : return "Coal consumption [kWh/a].";
				case 4 : return "Gas consumption [kWh/a].";
				case 5 : return "Electricity consumption [kWh/a].";
			} break;
			// LccSettings::intPara_t
			case 35 :
			switch (t) {
				case 0 : return "Price of Coal in ct per kWh.";
				case 1 : return "Price of Gas in ct per kWh.";
				case 2 : return "Price of Electricity in ct per kWh.";
			} break;
			// Material::para_t
			case 36 :
			switch (t) {
				case 0 : return "Dry density of the material.";
				case 1 : return "Specific heat capacity of the material.";
				case 2 : return "Thermal conductivity of the dry material.";
				case 3 : return "Vapor diffusion resistance factor.";
				case 4 : return "Water content in relation to 80% humidity.";
				case 5 : return "Water content at saturation.";
			} break;
			// Material::Category
			case 37 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "Coating";
				case 1 : if (no_description != nullptr) *no_description = true; return "Plaster";
				case 2 : if (no_description != nullptr) *no_description = true; return "Bricks";
				case 3 : if (no_description != nullptr) *no_description = true; return "NaturalStones";
				case 4 : if (no_description != nullptr) *no_description = true; return "Cementitious";
				case 5 : if (no_description != nullptr) *no_description = true; return "Insulations";
				case 6 : if (no_description != nullptr) *no_description = true; return "BuildingBoards";
				case 7 : if (no_description != nullptr) *no_description = true; return "Woodbased";
				case 8 : if (no_description != nullptr) *no_description = true; return "NaturalMaterials";
				case 9 : if (no_description != nullptr) *no_description = true; return "Soils";
				case 10 : if (no_description != nullptr) *no_description = true; return "CladdingSystems";
				case 11 : if (no_description != nullptr) *no_description = true; return "Foils";
				case 12 : if (no_description != nullptr) *no_description = true; return "Miscellaneous";
			} break;
			// MaterialLayer::para_t
			case 38 :
			switch (t) {
				case 0 : return "Thickness of the material layer.";
				case 1 : return "Lifetime of the material layer.";
			} break;
			// Network::PipeModel
			case 39 :
			switch (t) {
				case 0 : return "Pipe with a single fluid volume and with heat exchange";
				case 1 : return "Pipe with a discretized fluid volume and heat exchange";
			} break;
			// Network::ModelType
			case 40 :
			switch (t) {
				case 0 : return "Only Hydraulic calculation with constant temperature";
				case 1 : return "Thermo-hydraulic calculation";
			} break;
			// Network::NetworkType
			case 41 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "SinglePipe";
				case 1 : if (no_description != nullptr) *no_description = true; return "DoublePipe";
			} break;
			// Network::para_t
			case 42 :
			switch (t) {
				case 0 : return "Temperature for pipe dimensioning algorithm";
				case 1 : return "Temperature difference for pipe dimensioning algorithm";
				case 2 : return "Maximum pressure loss for pipe dimensioning algorithm";
				case 3 : return "Reference pressure applied to reference element";
				case 4 : return "Fluid temperature for hydraulic calculation, else initial temperature";
				case 5 : return "Initial Fluid temperature for thermo-hydraulic calculation";
				case 6 : return "Maximum discretization step for dynamic pipe model";
			} break;
			// NetworkBuriedPipeProperties::SoilType
			case 43 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "Sand";
				case 1 : if (no_description != nullptr) *no_description = true; return "Loam";
				case 2 : if (no_description != nullptr) *no_description = true; return "Silt";
			} break;
			// NetworkBuriedPipeProperties::para_t
			case 44 :
			switch (t) {
				case 0 : return "Spacing between supply and return pipes";
				case 1 : return "Distance between soil surface and pipes";
			} break;
			// NetworkComponent::ModelType
			case 45 :
			switch (t) {
				case 0 : return "Simple pipe with single fluid volume";
				case 1 : return "Detailled pipe with discretized fluid volume";
				case 2 : return "Pump with constant pressure head";
				case 3 : return "Pump with defined mass flux";
				case 4 : return "Pump with controlled pressure head";
				case 5 : return "Pump with linear pressure head";
				case 6 : return "Simple heat exchanger";
				case 7 : return "Simple heat pump installed at evaporator (source) side, based on carnot efficiency";
				case 8 : return "Simple heat pump installed at condenser (supply) side, based on carnot efficiency";
				case 9 : return "Detailled heat pump installed at evaporator (source) side, based on polynom for COP";
				case 10 : return "Detailled heat pump (on-off) at evaporator (source) source side, based on polynoms for heating power and el. power";
				case 11 : return "Controlled valve";
				case 12 : return "Valve with constant pressure loss";
				case 13 : return "Ideal heater with predefined supply temperature";
				case 14 : return "Adiabatic element with pressure loss defined by zeta-value";
			} break;
			// NetworkComponent::para_t
			case 46 :
			switch (t) {
				case 0 : return "Hydraulic diameter";
				case 1 : return "Pressure loss coefficient (zeta)";
				case 2 : return "Predefined pressure head";
				case 3 : return "Predefined mass flux";
				case 4 : return "Efficiency in optimal operation";
				case 5 : return "Fraction of losses that heats up fluid";
				case 6 : return "Maximum pressure head at minimal mass flow";
				case 7 : return "Maximum electrical power at optimal operation";
				case 8 : return "Design pressure head";
				case 9 : return "Design mass flux";
				case 10 : return "Pressure head reduction at zero mass flux";
				case 11 : return "Volume";
				case 12 : return "Discretization length";
				case 13 : return "Carnot effectiveness";
				case 14 : return "Maximum heating power";
				case 15 : return "Maximum cooling power";
				case 16 : return "Pressure loss";
				case 17 : return "Minimum outlet temperature";
				case 18 : if (no_description != nullptr) *no_description = true; return "HeatingPowerB0W35";
				case 19 : return "Heat pump supply temperature for heating buffer storage";
				case 20 : return "Heat pump return temperature for heating buffer storage";
				case 21 : return "Heat pump supply temperature for DHW buffer storage";
				case 22 : return "Heat pump return temperature for DHW buffer storage";
				case 23 : return "Heat pump heating buffer storage volume";
				case 24 : return "Heat pump DHW buffer storage volume";
				case 25 : return "Length of pipe";
			} break;
			// NetworkComponent::intPara_t
			case 47 :
			switch (t) {
				case 0 : return "Number of parallel pipes";
				case 1 : return "Number of parallel elements";
			} break;
			// NetworkComponent::ComponentCategory
			case 48 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "Pipes";
				case 1 : if (no_description != nullptr) *no_description = true; return "Pumps";
				case 2 : if (no_description != nullptr) *no_description = true; return "Heatpumps";
				case 3 : if (no_description != nullptr) *no_description = true; return "Other";
			} break;
			// NetworkController::ModelType
			case 49 :
			switch (t) {
				case 0 : return "Set points are given as constant parameters";
				case 1 : return "Scheduled set point values";
			} break;
			// NetworkController::ControlledProperty
			case 50 :
			switch (t) {
				case 0 : return "Control temperature difference of this element";
				case 1 : return "Control temperature difference of the following element";
				case 2 : return "Control zone thermostat values";
				case 3 : return "Control mass flux";
				case 4 : return "Control pump operation depending on following element";
				case 5 : return "Control pressure difference at worst point in the network";
			} break;
			// NetworkController::ControllerType
			case 51 :
			switch (t) {
				case 0 : return "PController";
				case 1 : return "PIController";
				case 2 : return "PIDController";
				case 3 : return "OnOffController";
			} break;
			// NetworkController::para_t
			case 52 :
			switch (t) {
				case 0 : return "Kp-parameter";
				case 1 : return "Ki-parameter";
				case 2 : return "Kd-parameter";
				case 3 : return "Temperature difference";
				case 4 : return "Mass flux";
				case 5 : return "Threshold value for pump operation";
				case 6 : return "Reset value for controller integral part";
				case 7 : return "Pressure difference";
			} break;
			// NetworkController::References
			case 53 :
			switch (t) {
				case 0 : return "ID of zone containing thermostat";
				case 1 : return "ID of schedule";
			} break;
			// NetworkFluid::para_t
			case 54 :
			switch (t) {
				case 0 : return "Dry density of the material.";
				case 1 : return "Specific heat capacity of the material.";
				case 2 : return "Thermal conductivity of the dry material.";
			} break;
			// NetworkHeatExchange::ModelType
			case 55 :
			switch (t) {
				case 0 : return "Constant temperature";
				case 1 : return "Time-dependent temperature";
				case 2 : return "Constant source temperature";
				case 3 : return "Time-dependent source temperature";
				case 4 : return "Zone air temperature";
				case 5 : return "Active construction layer (floor heating)";
				case 6 : return "Constant heat loss";
				case 7 : return "Time-dependent heat loss";
				case 8 : return "Constant heating demand";
				case 9 : return "Time-dependent heating demand";
				case 10 : return "Heating demand for space heating";
			} break;
			// NetworkHeatExchange::para_t
			case 56 :
			switch (t) {
				case 0 : return "Temperature for heat exchange";
				case 1 : return "Constant heat flux out of the element (heat loss)";
				case 2 : return "External heat transfer coeffient for the outside boundary";
				case 3 : return "FloorArea";
				case 4 : return "MaximumHeatingLoad";
				case 5 : return "HeatingEnergyDemand";
				case 6 : return "MaximumCoolingLoad";
				case 7 : return "CoolingEnergyDemand";
				case 8 : return "DomesticHotWaterDemand";
				case 9 : return "HeatingSupplyTemperature";
				case 10 : return "DomesticHotWaterSupplyTemperature";
			} break;
			// NetworkHeatExchange::BuildingType
			case 57 :
			switch (t) {
				case 0 : return "Single Family House";
				case 1 : return "Multi Family House";
				case 2 : return "Large Residential Building";
				case 3 : return "Office Building";
				case 4 : return "User Defined Building";
			} break;
			// NetworkHeatExchange::AmbientTemperatureType
			case 58 :
			switch (t) {
				case 0 : return "Undisturbed Soil Temperature";
				case 1 : return "Borehole Heat Exchanger";
				case 2 : return "User Defined";
			} break;
			// NetworkNode::NodeType
			case 59 :
			switch (t) {
				case 0 : if (no_description != nullptr) *no_description = true; return "SubStation";
				case 1 : if (no_description != nullptr) *no_description = true; return "Mixer";
				case 2 : if (no_description != nullptr) *no_description = true; return "Source";
			} break;
			// NetworkPipe::para_t
			case 60 :
			switch (t) {
				case 0 : return "Outer diameter (not including optional insulation)";
				case 1 : return "Pipe wall thickness";
				case 2 : return "Pipe wall surface roughness";
				case 3 : return "Thermal conductivity of pipe wall";
				case 4 : return "Specific heat capaciy of pipe wall";
				case 5 : return "Density of pipe wall";
				case 6 : return "Thickness of insulation around pipe";
				case 7 : return "Thermal conductivity of insulation";
				case 8 : return "Thickness of outer protection layer";
			} break;
			// OutputDefinition::timeType_t
			case 61 :
			switch (t) {
				case 0 : return "Write values as calculated at output times.";
				case 1 : return "Average values in time (mean value in output step).";
				case 2 : return "Integrate values in time.";
			} break;
			// Outputs::flag_t
			case 62 :
			switch (t) {
				case 0 : return "If true, output files are written in binary format (the default, if flag is missing).";
				case 1 : return "If true, default output definitions for zones are created.";
				case 2 : return "If true, default output definitions for networks are created.";
				case 3 : return "If true, default summation models and according output definitions for networks are created.";
			} break;
			// Room::para_t
			case 63 :
			switch (t) {
				case 0 : return "Floor usable area of the zone";
				case 1 : return "Volume of the zone";
				case 2 : return "Extra heat capacity";
			} break;
			// SubSurfaceComponent::SubSurfaceComponentType
			case 64 :
			switch (t) {
				case 0 : return "A window";
				case 1 : return "A door";
				case 2 : return "Some other component type";
			} break;
			// SubSurfaceComponent::para_t
			case 65 :
			switch (t) {
				case 0 : return "Reduction factor for dynamic shading";
			} break;
			// SupplySystem::SupplyType
			case 66 :
			switch (t) {
				case 0 : return "Stand-alone mode with given mass flux and suppply temperature";
				case 1 : return "User defined sub network";
				case 2 : return "Supply FMU loaded from a database and parametrized by the user";
				case 3 : return "User defined supply FMU";
			} break;
			// SupplySystem::para_t
			case 67 :
			switch (t) {
				case 0 : return "Maximum mass flux into the network, needed for pump control";
				case 1 : return "Constant supply temeprature";
				case 2 : return "Maximum mass flux towards the building.";
				case 3 : return "Procuder heating power";
			} break;
			// SurfaceHeating::para_t
			case 68 :
			switch (t) {
				case 0 : return "Heating limit";
				case 1 : return "Cooling limit";
				case 2 : return "Pipe spacing";
				case 3 : return "Maximum fluid velocity";
				case 4 : return "Temperature difference between supply and return fluid temperature";
			} break;
			// SurfaceHeating::Type
			case 69 :
			switch (t) {
				case 0 : return "Ideal surface conditioning";
				case 1 : return "Water-based surface conditioning: either ideal or part of a hydraulic network";
			} break;
			// VentilationNatural::para_t
			case 70 :
			switch (t) {
				case 0 : return "Air change rate";
			} break;
			// Window::Method
			case 71 :
			switch (t) {
				case 0 : return "None";
				case 1 : return "Fraction of area";
				case 2 : return "Constant width";
			} break;
			// Window::para_t
			case 72 :
			switch (t) {
				case 0 : return "Frame width of the window";
				case 1 : return "Frame area fraction of the window";
				case 2 : return "Divider width of the window";
				case 3 : return "Divider area fraction of the window";
			} break;
			// WindowDivider::para_t
			case 73 :
			switch (t) {
				case 0 : return "Divider material thickness.";
			} break;
			// WindowFrame::para_t
			case 74 :
			switch (t) {
				case 0 : return "Frame material thickness.";
			} break;
			// WindowGlazingSystem::modelType_t
			case 75 :
			switch (t) {
				case 0 : return "Standard globbed-layers model";
			} break;
			// WindowGlazingSystem::para_t
			case 76 :
			switch (t) {
				case 0 : return "Thermal transmittance";
			} break;
			// WindowGlazingSystem::splinePara_t
			case 77 :
			switch (t) {
				case 0 : return "Incidence-angle dependent short wave transmittance across glazing system";
			} break;
			// ZoneControlNaturalVentilation::para_t
			case 78 :
			switch (t) {
				case 0 : return "Maximum air change rate for natural ventilation to reach comfort conditions (if possible).";
				case 1 : return "Upper limit for room air temperature.";
				case 2 : return "Lower limit for room air temperature.";
				case 3 : return "Limit for wind speed .";
			} break;
			// ZoneControlShading::para_t
			case 79 :
			switch (t) {
				case 0 : return "Global horizontal (upper) sensor setpoint value.";
				case 1 : return "Global north (upper) sensor setpoint value.";
				case 2 : return "Global east (upper) sensor setpoint value.";
				case 3 : return "Global south (upper) sensor setpoint value.";
				case 4 : return "Global west (upper) sensor setpoint value.";
				case 5 : return "Dead band value for all sensors.";
			} break;
			// ZoneControlShading::Category
			case 80 :
			switch (t) {
				case 0 : return "One global horizontal sensor.";
				case 1 : return "One global horizontal and for each direction (N, E, S, W) a vertical sensor.";
			} break;
			// ZoneControlThermostat::para_t
			case 81 :
			switch (t) {
				case 0 : return "Thermostat tolerance heating and cooling mode";
				case 1 : return "Thermostat dead band (for digital controllers)";
			} break;
			// ZoneControlThermostat::ControlValue
			case 82 :
			switch (t) {
				case 0 : return "Air temperature";
				case 1 : return "Operative temperature";
			} break;
			// ZoneControlThermostat::ControllerType
			case 83 :
			switch (t) {
				case 0 : return "Analog";
				case 1 : return "Digital";
			} break;
			// ZoneIdealHeatingCooling::para_t
			case 84 :
			switch (t) {
				case 0 : return "Heating limit";
				case 1 : return "Cooling limit (positive)";
			} break;
			// ZoneTemplate::SubTemplateType
			case 85 :
			switch (t) {
				case 0 : return "Person loads";
				case 1 : return "Equipment loads";
				case 2 : return "Lighting loads";
				case 3 : return "Other internal loads";
				case 4 : return "Thermostat control";
				case 5 : return "Shading control";
				case 6 : return "Natural ventilation control";
				case 7 : return "Infiltration loads";
				case 8 : if (no_description != nullptr) *no_description = true; return "NaturalVentilation";
				case 9 : return "Heating/cooling loads";
			} break;
		} // switch
		throw IBK::Exception(IBK::FormatString("Cannot determine description for enumeration type '%1' and index '%2'.")
			.arg(enumtype).arg(t), "[KeywordList::Description]");
	}

	const char * KeywordList::Unit(const char * const enumtype, int t) {
		switch (enum2index(enumtype)) {
			// AcousticBuildingTemplate::AcousticBuildingType
			case 0 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
			} break;
			// AcousticComponent::para_t
			case 1 :
			switch (t) {
				case 0 : return "m2";
				case 1 : return "%";
			} break;
			// AcousticComponent::SizeMethod
			case 2 :
			switch (t) {
				case 0 : return "m2";
				case 1 : return "-";
			} break;
			// AcousticReferenceComponent::RequirementType
			case 3 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// AcousticReferenceComponent::ComponentType
			case 4 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
			} break;
			// AcousticSoundAbsorptionPartition::para_t
			case 5 :
			switch (t) {
				case 0 : return "---";
			} break;
			// AcousticSoundProtectionTemplate::AcousticBuildingType
			case 6 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
			} break;
			// AcousticTemplate::splinePara_t
			case 7 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
			} break;
			// Component::ComponentType
			case 8 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
				case 7 : return "";
				case 8 : return "";
				case 9 : return "";
				case 10 : return "";
				case 11 : return "";
			} break;
			// Construction::UsageType
			case 9 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
				case 7 : return "";
				case 8 : return "";
			} break;
			// Construction::InsulationKind
			case 10 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
			} break;
			// Construction::MaterialKind
			case 11 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
			} break;
			// Construction::para_t
			case 12 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
			} break;
			// EpdCategorySet::Category
			case 13 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
				case 2 : return "-";
				case 3 : return "-";
			} break;
			// EpdDataset::Type
			case 14 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
			} break;
			// EpdDataset::Category
			case 15 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
			} break;
			// EpdModuleDataset::para_t
			case 16 :
			switch (t) {
				case 0 : return "kg/m2";
				case 1 : return "kg/m3";
				case 2 : return "kg";
				case 3 : return "kg";
				case 4 : return "kg";
				case 5 : return "kg";
				case 6 : return "kg";
				case 7 : return "W/mK";
				case 8 : return "W/mK";
			} break;
			// EpdModuleDataset::Module
			case 17 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
				case 7 : return "";
				case 8 : return "";
				case 9 : return "";
				case 10 : return "";
				case 11 : return "";
				case 12 : return "";
				case 13 : return "";
				case 14 : return "";
				case 15 : return "";
				case 16 : return "";
			} break;
			// Infiltration::para_t
			case 18 :
			switch (t) {
				case 0 : return "1/h";
				case 1 : return "-";
			} break;
			// Infiltration::AirChangeType
			case 19 :
			switch (t) {
				case 0 : return "1/h";
				case 1 : return "1/h";
			} break;
			// InterfaceHeatConduction::modelType_t
			case 20 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// InterfaceHeatConduction::OtherZoneType
			case 21 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// InterfaceHeatConduction::para_t
			case 22 :
			switch (t) {
				case 0 : return "W/m2K";
				case 1 : return "C";
			} break;
			// InternalLoad::para_t
			case 23 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "Person/m2";
				case 2 : return "m2/Person";
				case 3 : return "W";
				case 4 : return "W/m2";
				case 5 : return "kg/m2s";
				case 6 : return "---";
				case 7 : return "---";
				case 8 : return "---";
			} break;
			// InternalLoad::Category
			case 24 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
				case 2 : return "-";
				case 3 : return "-";
			} break;
			// InternalLoad::PersonCountMethod
			case 25 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
				case 2 : return "-";
			} break;
			// InternalLoad::PowerMethod
			case 26 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
			} break;
			// KeywordList::MyParameters
			case 27 :
			switch (t) {
				case 0 : return "C";
				case 1 : return "kg";
			} break;
			// LcaSettings::para_t
			case 28 :
			switch (t) {
				case 0 : return "a";
				case 1 : return "-";
				case 2 : return "m2";
			} break;
			// LcaSettings::Module
			case 29 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
				case 7 : return "";
				case 8 : return "";
				case 9 : return "";
				case 10 : return "";
				case 11 : return "";
				case 12 : return "";
				case 13 : return "";
				case 14 : return "";
				case 15 : return "";
				case 16 : return "";
			} break;
			// LcaSettings::LcaCategory
			case 30 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
			} break;
			// LcaSettings::UsageType
			case 31 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// LcaSettings::CalculationMode
			case 32 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// LcaSettings::CertificationSytem
			case 33 :
			switch (t) {
				case 0 : return "";
			} break;
			// LccSettings::para_t
			case 34 :
			switch (t) {
				case 0 : return "%";
				case 1 : return "%";
				case 2 : return "%";
				case 3 : return "kWh/a";
				case 4 : return "kWh/a";
				case 5 : return "kWh/a";
			} break;
			// LccSettings::intPara_t
			case 35 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// Material::para_t
			case 36 :
			switch (t) {
				case 0 : return "kg/m3";
				case 1 : return "J/kgK";
				case 2 : return "W/mK";
				case 3 : return "-";
				case 4 : return "kg/m3";
				case 5 : return "kg/m3";
			} break;
			// Material::Category
			case 37 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
				case 7 : return "";
				case 8 : return "";
				case 9 : return "";
				case 10 : return "";
				case 11 : return "";
				case 12 : return "";
			} break;
			// MaterialLayer::para_t
			case 38 :
			switch (t) {
				case 0 : return "m";
				case 1 : return "a";
			} break;
			// Network::PipeModel
			case 39 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// Network::ModelType
			case 40 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// Network::NetworkType
			case 41 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// Network::para_t
			case 42 :
			switch (t) {
				case 0 : return "C";
				case 1 : return "K";
				case 2 : return "Pa/m";
				case 3 : return "Pa";
				case 4 : return "C";
				case 5 : return "C";
				case 6 : return "m";
			} break;
			// NetworkBuriedPipeProperties::SoilType
			case 43 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// NetworkBuriedPipeProperties::para_t
			case 44 :
			switch (t) {
				case 0 : return "m";
				case 1 : return "m";
			} break;
			// NetworkComponent::ModelType
			case 45 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
				case 7 : return "";
				case 8 : return "";
				case 9 : return "";
				case 10 : return "";
				case 11 : return "";
				case 12 : return "";
				case 13 : return "";
				case 14 : return "";
			} break;
			// NetworkComponent::para_t
			case 46 :
			switch (t) {
				case 0 : return "mm";
				case 1 : return "---";
				case 2 : return "Bar";
				case 3 : return "kg/s";
				case 4 : return "---";
				case 5 : return "---";
				case 6 : return "Bar";
				case 7 : return "kW";
				case 8 : return "Bar";
				case 9 : return "kg/s";
				case 10 : return "---";
				case 11 : return "m3";
				case 12 : return "m";
				case 13 : return "---";
				case 14 : return "kW";
				case 15 : return "kW";
				case 16 : return "Bar";
				case 17 : return "C";
				case 18 : return "kW";
				case 19 : return "C";
				case 20 : return "C";
				case 21 : return "C";
				case 22 : return "C";
				case 23 : return "l";
				case 24 : return "l";
				case 25 : return "m";
			} break;
			// NetworkComponent::intPara_t
			case 47 :
			switch (t) {
				case 0 : return "---";
				case 1 : return "---";
			} break;
			// NetworkComponent::ComponentCategory
			case 48 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
			} break;
			// NetworkController::ModelType
			case 49 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// NetworkController::ControlledProperty
			case 50 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
			} break;
			// NetworkController::ControllerType
			case 51 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
			} break;
			// NetworkController::para_t
			case 52 :
			switch (t) {
				case 0 : return "---";
				case 1 : return "---";
				case 2 : return "---";
				case 3 : return "K";
				case 4 : return "kg/s";
				case 5 : return "W";
				case 6 : return "---";
				case 7 : return "Pa";
			} break;
			// NetworkController::References
			case 53 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
			} break;
			// NetworkFluid::para_t
			case 54 :
			switch (t) {
				case 0 : return "kg/m3";
				case 1 : return "J/kgK";
				case 2 : return "W/mK";
			} break;
			// NetworkHeatExchange::ModelType
			case 55 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
				case 7 : return "";
				case 8 : return "";
				case 9 : return "";
				case 10 : return "";
			} break;
			// NetworkHeatExchange::para_t
			case 56 :
			switch (t) {
				case 0 : return "C";
				case 1 : return "W";
				case 2 : return "W/m2K";
				case 3 : return "m2";
				case 4 : return "kW";
				case 5 : return "kWh";
				case 6 : return "kW";
				case 7 : return "kWh";
				case 8 : return "kWh";
				case 9 : return "C";
				case 10 : return "C";
			} break;
			// NetworkHeatExchange::BuildingType
			case 57 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
			} break;
			// NetworkHeatExchange::AmbientTemperatureType
			case 58 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// NetworkNode::NodeType
			case 59 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// NetworkPipe::para_t
			case 60 :
			switch (t) {
				case 0 : return "mm";
				case 1 : return "mm";
				case 2 : return "mm";
				case 3 : return "W/mK";
				case 4 : return "J/kgK";
				case 5 : return "kg/m3";
				case 6 : return "mm";
				case 7 : return "W/mK";
				case 8 : return "mm";
			} break;
			// OutputDefinition::timeType_t
			case 61 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// Outputs::flag_t
			case 62 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
			} break;
			// Room::para_t
			case 63 :
			switch (t) {
				case 0 : return "m2";
				case 1 : return "m3";
				case 2 : return "J/K";
			} break;
			// SubSurfaceComponent::SubSurfaceComponentType
			case 64 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// SubSurfaceComponent::para_t
			case 65 :
			switch (t) {
				case 0 : return "---";
			} break;
			// SupplySystem::SupplyType
			case 66 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
			} break;
			// SupplySystem::para_t
			case 67 :
			switch (t) {
				case 0 : return "kg/s";
				case 1 : return "C";
				case 2 : return "kg/s";
				case 3 : return "W";
			} break;
			// SurfaceHeating::para_t
			case 68 :
			switch (t) {
				case 0 : return "W/m2";
				case 1 : return "W/m2";
				case 2 : return "m";
				case 3 : return "m/s";
				case 4 : return "K";
			} break;
			// SurfaceHeating::Type
			case 69 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// VentilationNatural::para_t
			case 70 :
			switch (t) {
				case 0 : return "1/h";
			} break;
			// Window::Method
			case 71 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
			} break;
			// Window::para_t
			case 72 :
			switch (t) {
				case 0 : return "m";
				case 1 : return "---";
				case 2 : return "m";
				case 3 : return "---";
			} break;
			// WindowDivider::para_t
			case 73 :
			switch (t) {
				case 0 : return "m";
			} break;
			// WindowFrame::para_t
			case 74 :
			switch (t) {
				case 0 : return "m";
			} break;
			// WindowGlazingSystem::modelType_t
			case 75 :
			switch (t) {
				case 0 : return "";
			} break;
			// WindowGlazingSystem::para_t
			case 76 :
			switch (t) {
				case 0 : return "W/m2K";
			} break;
			// WindowGlazingSystem::splinePara_t
			case 77 :
			switch (t) {
				case 0 : return "---";
			} break;
			// ZoneControlNaturalVentilation::para_t
			case 78 :
			switch (t) {
				case 0 : return "1/h";
				case 1 : return "C";
				case 2 : return "C";
				case 3 : return "m/s";
			} break;
			// ZoneControlShading::para_t
			case 79 :
			switch (t) {
				case 0 : return "W/m2";
				case 1 : return "W/m2";
				case 2 : return "W/m2";
				case 3 : return "W/m2";
				case 4 : return "W/m2";
				case 5 : return "W/m2";
			} break;
			// ZoneControlShading::Category
			case 80 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
			} break;
			// ZoneControlThermostat::para_t
			case 81 :
			switch (t) {
				case 0 : return "K";
				case 1 : return "K";
			} break;
			// ZoneControlThermostat::ControlValue
			case 82 :
			switch (t) {
				case 0 : return "-";
				case 1 : return "-";
			} break;
			// ZoneControlThermostat::ControllerType
			case 83 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
			} break;
			// ZoneIdealHeatingCooling::para_t
			case 84 :
			switch (t) {
				case 0 : return "W/m2";
				case 1 : return "W/m2";
			} break;
			// ZoneTemplate::SubTemplateType
			case 85 :
			switch (t) {
				case 0 : return "";
				case 1 : return "";
				case 2 : return "";
				case 3 : return "";
				case 4 : return "";
				case 5 : return "";
				case 6 : return "";
				case 7 : return "";
				case 8 : return "";
				case 9 : return "";
			} break;
		} // switch
		throw IBK::Exception(IBK::FormatString("Cannot determine default unit for enumeration type '%1' and index '%2'.")
			.arg(enumtype).arg(t), "[KeywordList::Unit]");
	}

	const char * KeywordList::Color(const char * const enumtype, int t) {
		switch (enum2index(enumtype)) {
			// AcousticBuildingTemplate::AcousticBuildingType
			case 0 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
			} break;
			// AcousticComponent::para_t
			case 1 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// AcousticComponent::SizeMethod
			case 2 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// AcousticReferenceComponent::RequirementType
			case 3 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// AcousticReferenceComponent::ComponentType
			case 4 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// AcousticSoundAbsorptionPartition::para_t
			case 5 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// AcousticSoundProtectionTemplate::AcousticBuildingType
			case 6 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
			} break;
			// AcousticTemplate::splinePara_t
			case 7 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// Component::ComponentType
			case 8 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
				case 9 : return "#FFFFFF";
				case 10 : return "#FFFFFF";
				case 11 : return "#FFFFFF";
			} break;
			// Construction::UsageType
			case 9 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
			} break;
			// Construction::InsulationKind
			case 10 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
			} break;
			// Construction::MaterialKind
			case 11 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
			} break;
			// Construction::para_t
			case 12 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// EpdCategorySet::Category
			case 13 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// EpdDataset::Type
			case 14 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
			} break;
			// EpdDataset::Category
			case 15 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// EpdModuleDataset::para_t
			case 16 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
			} break;
			// EpdModuleDataset::Module
			case 17 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
				case 9 : return "#FFFFFF";
				case 10 : return "#FFFFFF";
				case 11 : return "#FFFFFF";
				case 12 : return "#FFFFFF";
				case 13 : return "#FFFFFF";
				case 14 : return "#FFFFFF";
				case 15 : return "#FFFFFF";
				case 16 : return "#FFFFFF";
			} break;
			// Infiltration::para_t
			case 18 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// Infiltration::AirChangeType
			case 19 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// InterfaceHeatConduction::modelType_t
			case 20 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// InterfaceHeatConduction::OtherZoneType
			case 21 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// InterfaceHeatConduction::para_t
			case 22 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// InternalLoad::para_t
			case 23 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
			} break;
			// InternalLoad::Category
			case 24 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// InternalLoad::PersonCountMethod
			case 25 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// InternalLoad::PowerMethod
			case 26 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// KeywordList::MyParameters
			case 27 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// LcaSettings::para_t
			case 28 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// LcaSettings::Module
			case 29 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
				case 9 : return "#FFFFFF";
				case 10 : return "#FFFFFF";
				case 11 : return "#FFFFFF";
				case 12 : return "#FFFFFF";
				case 13 : return "#FFFFFF";
				case 14 : return "#FFFFFF";
				case 15 : return "#FFFFFF";
				case 16 : return "#FFFFFF";
			} break;
			// LcaSettings::LcaCategory
			case 30 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// LcaSettings::UsageType
			case 31 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// LcaSettings::CalculationMode
			case 32 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// LcaSettings::CertificationSytem
			case 33 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// LccSettings::para_t
			case 34 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
			} break;
			// LccSettings::intPara_t
			case 35 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// Material::para_t
			case 36 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
			} break;
			// Material::Category
			case 37 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
				case 9 : return "#FFFFFF";
				case 10 : return "#FFFFFF";
				case 11 : return "#FFFFFF";
				case 12 : return "#FFFFFF";
			} break;
			// MaterialLayer::para_t
			case 38 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// Network::PipeModel
			case 39 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// Network::ModelType
			case 40 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// Network::NetworkType
			case 41 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// Network::para_t
			case 42 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
			} break;
			// NetworkBuriedPipeProperties::SoilType
			case 43 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// NetworkBuriedPipeProperties::para_t
			case 44 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// NetworkComponent::ModelType
			case 45 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
				case 9 : return "#FFFFFF";
				case 10 : return "#FFFFFF";
				case 11 : return "#FFFFFF";
				case 12 : return "#FFFFFF";
				case 13 : return "#FFFFFF";
				case 14 : return "#FFFFFF";
			} break;
			// NetworkComponent::para_t
			case 46 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
				case 9 : return "#FFFFFF";
				case 10 : return "#FFFFFF";
				case 11 : return "#FFFFFF";
				case 12 : return "#FFFFFF";
				case 13 : return "#FFFFFF";
				case 14 : return "#FFFFFF";
				case 15 : return "#FFFFFF";
				case 16 : return "#FFFFFF";
				case 17 : return "#FFFFFF";
				case 18 : return "#FFFFFF";
				case 19 : return "#FFFFFF";
				case 20 : return "#FFFFFF";
				case 21 : return "#FFFFFF";
				case 22 : return "#FFFFFF";
				case 23 : return "#FFFFFF";
				case 24 : return "#FFFFFF";
				case 25 : return "#FFFFFF";
			} break;
			// NetworkComponent::intPara_t
			case 47 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// NetworkComponent::ComponentCategory
			case 48 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// NetworkController::ModelType
			case 49 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// NetworkController::ControlledProperty
			case 50 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
			} break;
			// NetworkController::ControllerType
			case 51 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// NetworkController::para_t
			case 52 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
			} break;
			// NetworkController::References
			case 53 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// NetworkFluid::para_t
			case 54 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// NetworkHeatExchange::ModelType
			case 55 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
				case 9 : return "#FFFFFF";
				case 10 : return "#FFFFFF";
			} break;
			// NetworkHeatExchange::para_t
			case 56 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
				case 9 : return "#FFFFFF";
				case 10 : return "#FFFFFF";
			} break;
			// NetworkHeatExchange::BuildingType
			case 57 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
			} break;
			// NetworkHeatExchange::AmbientTemperatureType
			case 58 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// NetworkNode::NodeType
			case 59 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// NetworkPipe::para_t
			case 60 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
				case 6 : return "#FFFFFF";
				case 7 : return "#FFFFFF";
				case 8 : return "#FFFFFF";
			} break;
			// OutputDefinition::timeType_t
			case 61 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// Outputs::flag_t
			case 62 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// Room::para_t
			case 63 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// SubSurfaceComponent::SubSurfaceComponentType
			case 64 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// SubSurfaceComponent::para_t
			case 65 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// SupplySystem::SupplyType
			case 66 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// SupplySystem::para_t
			case 67 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// SurfaceHeating::para_t
			case 68 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
			} break;
			// SurfaceHeating::Type
			case 69 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// VentilationNatural::para_t
			case 70 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// Window::Method
			case 71 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
			} break;
			// Window::para_t
			case 72 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// WindowDivider::para_t
			case 73 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// WindowFrame::para_t
			case 74 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// WindowGlazingSystem::modelType_t
			case 75 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// WindowGlazingSystem::para_t
			case 76 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// WindowGlazingSystem::splinePara_t
			case 77 :
			switch (t) {
				case 0 : return "#FFFFFF";
			} break;
			// ZoneControlNaturalVentilation::para_t
			case 78 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
			} break;
			// ZoneControlShading::para_t
			case 79 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
				case 2 : return "#FFFFFF";
				case 3 : return "#FFFFFF";
				case 4 : return "#FFFFFF";
				case 5 : return "#FFFFFF";
			} break;
			// ZoneControlShading::Category
			case 80 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// ZoneControlThermostat::para_t
			case 81 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// ZoneControlThermostat::ControlValue
			case 82 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// ZoneControlThermostat::ControllerType
			case 83 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// ZoneIdealHeatingCooling::para_t
			case 84 :
			switch (t) {
				case 0 : return "#FFFFFF";
				case 1 : return "#FFFFFF";
			} break;
			// ZoneTemplate::SubTemplateType
			case 85 :
			switch (t) {
				case 0 : return "#FFBB55";
				case 1 : return "#AA2222";
				case 2 : return "#FFEECC";
				case 3 : return "#602222";
				case 4 : return "#E00010";
				case 5 : return "#A2F0F4";
				case 6 : return "#00A000";
				case 7 : return "#A0B0FF";
				case 8 : return "#22EE22";
				case 9 : return "#B08000";
			} break;
		} // switch
		throw IBK::Exception(IBK::FormatString("Cannot determine color for enumeration type '%1' and index '%2'.")
			.arg(enumtype).arg(t), "[KeywordList::Color]");
	}

	double KeywordList::DefaultValue(const char * const enumtype, int t) {
		switch (enum2index(enumtype)) {
			// AcousticBuildingTemplate::AcousticBuildingType
			case 0 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// AcousticComponent::para_t
			case 1 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// AcousticComponent::SizeMethod
			case 2 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// AcousticReferenceComponent::RequirementType
			case 3 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// AcousticReferenceComponent::ComponentType
			case 4 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// AcousticSoundAbsorptionPartition::para_t
			case 5 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// AcousticSoundProtectionTemplate::AcousticBuildingType
			case 6 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// AcousticTemplate::splinePara_t
			case 7 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Component::ComponentType
			case 8 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
				case 10 : return std::numeric_limits<double>::quiet_NaN();
				case 11 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Construction::UsageType
			case 9 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Construction::InsulationKind
			case 10 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Construction::MaterialKind
			case 11 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Construction::para_t
			case 12 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// EpdCategorySet::Category
			case 13 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// EpdDataset::Type
			case 14 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// EpdDataset::Category
			case 15 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// EpdModuleDataset::para_t
			case 16 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// EpdModuleDataset::Module
			case 17 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
				case 10 : return std::numeric_limits<double>::quiet_NaN();
				case 11 : return std::numeric_limits<double>::quiet_NaN();
				case 12 : return std::numeric_limits<double>::quiet_NaN();
				case 13 : return std::numeric_limits<double>::quiet_NaN();
				case 14 : return std::numeric_limits<double>::quiet_NaN();
				case 15 : return std::numeric_limits<double>::quiet_NaN();
				case 16 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Infiltration::para_t
			case 18 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Infiltration::AirChangeType
			case 19 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// InterfaceHeatConduction::modelType_t
			case 20 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// InterfaceHeatConduction::OtherZoneType
			case 21 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// InterfaceHeatConduction::para_t
			case 22 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// InternalLoad::para_t
			case 23 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// InternalLoad::Category
			case 24 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// InternalLoad::PersonCountMethod
			case 25 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// InternalLoad::PowerMethod
			case 26 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// KeywordList::MyParameters
			case 27 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// LcaSettings::para_t
			case 28 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// LcaSettings::Module
			case 29 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
				case 10 : return std::numeric_limits<double>::quiet_NaN();
				case 11 : return std::numeric_limits<double>::quiet_NaN();
				case 12 : return std::numeric_limits<double>::quiet_NaN();
				case 13 : return std::numeric_limits<double>::quiet_NaN();
				case 14 : return std::numeric_limits<double>::quiet_NaN();
				case 15 : return std::numeric_limits<double>::quiet_NaN();
				case 16 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// LcaSettings::LcaCategory
			case 30 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// LcaSettings::UsageType
			case 31 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// LcaSettings::CalculationMode
			case 32 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// LcaSettings::CertificationSytem
			case 33 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// LccSettings::para_t
			case 34 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// LccSettings::intPara_t
			case 35 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Material::para_t
			case 36 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Material::Category
			case 37 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
				case 10 : return std::numeric_limits<double>::quiet_NaN();
				case 11 : return std::numeric_limits<double>::quiet_NaN();
				case 12 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// MaterialLayer::para_t
			case 38 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Network::PipeModel
			case 39 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Network::ModelType
			case 40 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Network::NetworkType
			case 41 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Network::para_t
			case 42 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkBuriedPipeProperties::SoilType
			case 43 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkBuriedPipeProperties::para_t
			case 44 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkComponent::ModelType
			case 45 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
				case 10 : return std::numeric_limits<double>::quiet_NaN();
				case 11 : return std::numeric_limits<double>::quiet_NaN();
				case 12 : return std::numeric_limits<double>::quiet_NaN();
				case 13 : return std::numeric_limits<double>::quiet_NaN();
				case 14 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkComponent::para_t
			case 46 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
				case 10 : return std::numeric_limits<double>::quiet_NaN();
				case 11 : return std::numeric_limits<double>::quiet_NaN();
				case 12 : return std::numeric_limits<double>::quiet_NaN();
				case 13 : return std::numeric_limits<double>::quiet_NaN();
				case 14 : return std::numeric_limits<double>::quiet_NaN();
				case 15 : return std::numeric_limits<double>::quiet_NaN();
				case 16 : return std::numeric_limits<double>::quiet_NaN();
				case 17 : return std::numeric_limits<double>::quiet_NaN();
				case 18 : return std::numeric_limits<double>::quiet_NaN();
				case 19 : return std::numeric_limits<double>::quiet_NaN();
				case 20 : return std::numeric_limits<double>::quiet_NaN();
				case 21 : return std::numeric_limits<double>::quiet_NaN();
				case 22 : return std::numeric_limits<double>::quiet_NaN();
				case 23 : return std::numeric_limits<double>::quiet_NaN();
				case 24 : return std::numeric_limits<double>::quiet_NaN();
				case 25 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkComponent::intPara_t
			case 47 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkComponent::ComponentCategory
			case 48 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkController::ModelType
			case 49 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkController::ControlledProperty
			case 50 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkController::ControllerType
			case 51 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkController::para_t
			case 52 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkController::References
			case 53 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkFluid::para_t
			case 54 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkHeatExchange::ModelType
			case 55 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
				case 10 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkHeatExchange::para_t
			case 56 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
				case 10 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkHeatExchange::BuildingType
			case 57 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkHeatExchange::AmbientTemperatureType
			case 58 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkNode::NodeType
			case 59 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// NetworkPipe::para_t
			case 60 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// OutputDefinition::timeType_t
			case 61 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Outputs::flag_t
			case 62 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Room::para_t
			case 63 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// SubSurfaceComponent::SubSurfaceComponentType
			case 64 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// SubSurfaceComponent::para_t
			case 65 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// SupplySystem::SupplyType
			case 66 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// SupplySystem::para_t
			case 67 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// SurfaceHeating::para_t
			case 68 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// SurfaceHeating::Type
			case 69 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// VentilationNatural::para_t
			case 70 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Window::Method
			case 71 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// Window::para_t
			case 72 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// WindowDivider::para_t
			case 73 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// WindowFrame::para_t
			case 74 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// WindowGlazingSystem::modelType_t
			case 75 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// WindowGlazingSystem::para_t
			case 76 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// WindowGlazingSystem::splinePara_t
			case 77 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// ZoneControlNaturalVentilation::para_t
			case 78 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// ZoneControlShading::para_t
			case 79 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// ZoneControlShading::Category
			case 80 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// ZoneControlThermostat::para_t
			case 81 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// ZoneControlThermostat::ControlValue
			case 82 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// ZoneControlThermostat::ControllerType
			case 83 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// ZoneIdealHeatingCooling::para_t
			case 84 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
			} break;
			// ZoneTemplate::SubTemplateType
			case 85 :
			switch (t) {
				case 0 : return std::numeric_limits<double>::quiet_NaN();
				case 1 : return std::numeric_limits<double>::quiet_NaN();
				case 2 : return std::numeric_limits<double>::quiet_NaN();
				case 3 : return std::numeric_limits<double>::quiet_NaN();
				case 4 : return std::numeric_limits<double>::quiet_NaN();
				case 5 : return std::numeric_limits<double>::quiet_NaN();
				case 6 : return std::numeric_limits<double>::quiet_NaN();
				case 7 : return std::numeric_limits<double>::quiet_NaN();
				case 8 : return std::numeric_limits<double>::quiet_NaN();
				case 9 : return std::numeric_limits<double>::quiet_NaN();
			} break;
		} // switch
		throw IBK::Exception(IBK::FormatString("Cannot determine default value for enumeration type '%1' and index '%2'.")
			.arg(enumtype).arg(t), "[KeywordList::DefaultValue]");
	}

	// number of entries in a keyword list
	unsigned int KeywordList::Count(const char * const enumtype) {
		switch (enum2index(enumtype)) {
			// AcousticBuildingTemplate::AcousticBuildingType
			case 0 : return 6;
			// AcousticComponent::para_t
			case 1 : return 2;
			// AcousticComponent::SizeMethod
			case 2 : return 2;
			// AcousticReferenceComponent::RequirementType
			case 3 : return 2;
			// AcousticReferenceComponent::ComponentType
			case 4 : return 4;
			// AcousticSoundAbsorptionPartition::para_t
			case 5 : return 1;
			// AcousticSoundProtectionTemplate::AcousticBuildingType
			case 6 : return 6;
			// AcousticTemplate::splinePara_t
			case 7 : return 2;
			// Component::ComponentType
			case 8 : return 12;
			// Construction::UsageType
			case 9 : return 9;
			// Construction::InsulationKind
			case 10 : return 5;
			// Construction::MaterialKind
			case 11 : return 7;
			// Construction::para_t
			case 12 : return 2;
			// EpdCategorySet::Category
			case 13 : return 4;
			// EpdDataset::Type
			case 14 : return 5;
			// EpdDataset::Category
			case 15 : return 4;
			// EpdModuleDataset::para_t
			case 16 : return 9;
			// EpdModuleDataset::Module
			case 17 : return 17;
			// Infiltration::para_t
			case 18 : return 2;
			// Infiltration::AirChangeType
			case 19 : return 2;
			// InterfaceHeatConduction::modelType_t
			case 20 : return 2;
			// InterfaceHeatConduction::OtherZoneType
			case 21 : return 3;
			// InterfaceHeatConduction::para_t
			case 22 : return 2;
			// InternalLoad::para_t
			case 23 : return 9;
			// InternalLoad::Category
			case 24 : return 4;
			// InternalLoad::PersonCountMethod
			case 25 : return 3;
			// InternalLoad::PowerMethod
			case 26 : return 2;
			// KeywordList::MyParameters
			case 27 : return 2;
			// LcaSettings::para_t
			case 28 : return 3;
			// LcaSettings::Module
			case 29 : return 17;
			// LcaSettings::LcaCategory
			case 30 : return 4;
			// LcaSettings::UsageType
			case 31 : return 3;
			// LcaSettings::CalculationMode
			case 32 : return 2;
			// LcaSettings::CertificationSytem
			case 33 : return 1;
			// LccSettings::para_t
			case 34 : return 6;
			// LccSettings::intPara_t
			case 35 : return 3;
			// Material::para_t
			case 36 : return 6;
			// Material::Category
			case 37 : return 13;
			// MaterialLayer::para_t
			case 38 : return 2;
			// Network::PipeModel
			case 39 : return 2;
			// Network::ModelType
			case 40 : return 2;
			// Network::NetworkType
			case 41 : return 2;
			// Network::para_t
			case 42 : return 7;
			// NetworkBuriedPipeProperties::SoilType
			case 43 : return 3;
			// NetworkBuriedPipeProperties::para_t
			case 44 : return 2;
			// NetworkComponent::ModelType
			case 45 : return 15;
			// NetworkComponent::para_t
			case 46 : return 26;
			// NetworkComponent::intPara_t
			case 47 : return 2;
			// NetworkComponent::ComponentCategory
			case 48 : return 4;
			// NetworkController::ModelType
			case 49 : return 2;
			// NetworkController::ControlledProperty
			case 50 : return 6;
			// NetworkController::ControllerType
			case 51 : return 4;
			// NetworkController::para_t
			case 52 : return 8;
			// NetworkController::References
			case 53 : return 2;
			// NetworkFluid::para_t
			case 54 : return 3;
			// NetworkHeatExchange::ModelType
			case 55 : return 11;
			// NetworkHeatExchange::para_t
			case 56 : return 11;
			// NetworkHeatExchange::BuildingType
			case 57 : return 5;
			// NetworkHeatExchange::AmbientTemperatureType
			case 58 : return 3;
			// NetworkNode::NodeType
			case 59 : return 3;
			// NetworkPipe::para_t
			case 60 : return 9;
			// OutputDefinition::timeType_t
			case 61 : return 3;
			// Outputs::flag_t
			case 62 : return 4;
			// Room::para_t
			case 63 : return 3;
			// SubSurfaceComponent::SubSurfaceComponentType
			case 64 : return 3;
			// SubSurfaceComponent::para_t
			case 65 : return 1;
			// SupplySystem::SupplyType
			case 66 : return 4;
			// SupplySystem::para_t
			case 67 : return 4;
			// SurfaceHeating::para_t
			case 68 : return 5;
			// SurfaceHeating::Type
			case 69 : return 2;
			// VentilationNatural::para_t
			case 70 : return 1;
			// Window::Method
			case 71 : return 3;
			// Window::para_t
			case 72 : return 4;
			// WindowDivider::para_t
			case 73 : return 1;
			// WindowFrame::para_t
			case 74 : return 1;
			// WindowGlazingSystem::modelType_t
			case 75 : return 1;
			// WindowGlazingSystem::para_t
			case 76 : return 1;
			// WindowGlazingSystem::splinePara_t
			case 77 : return 1;
			// ZoneControlNaturalVentilation::para_t
			case 78 : return 4;
			// ZoneControlShading::para_t
			case 79 : return 6;
			// ZoneControlShading::Category
			case 80 : return 2;
			// ZoneControlThermostat::para_t
			case 81 : return 2;
			// ZoneControlThermostat::ControlValue
			case 82 : return 2;
			// ZoneControlThermostat::ControllerType
			case 83 : return 2;
			// ZoneIdealHeatingCooling::para_t
			case 84 : return 2;
			// ZoneTemplate::SubTemplateType
			case 85 : return 10;
		} // switch
		throw IBK::Exception(IBK::FormatString("Invalid enumeration type '%1'.")
			.arg(enumtype), "[KeywordList::Count]");
	}

	// max index for entries sharing a category in a keyword list
	int KeywordList::MaxIndex(const char * const enumtype) {
		switch (enum2index(enumtype)) {
			// AcousticBuildingTemplate::AcousticBuildingType
			case 0 : return 5;
			// AcousticComponent::para_t
			case 1 : return 1;
			// AcousticComponent::SizeMethod
			case 2 : return 1;
			// AcousticReferenceComponent::RequirementType
			case 3 : return 1;
			// AcousticReferenceComponent::ComponentType
			case 4 : return 3;
			// AcousticSoundAbsorptionPartition::para_t
			case 5 : return 0;
			// AcousticSoundProtectionTemplate::AcousticBuildingType
			case 6 : return 5;
			// AcousticTemplate::splinePara_t
			case 7 : return 1;
			// Component::ComponentType
			case 8 : return 11;
			// Construction::UsageType
			case 9 : return 8;
			// Construction::InsulationKind
			case 10 : return 4;
			// Construction::MaterialKind
			case 11 : return 6;
			// Construction::para_t
			case 12 : return 1;
			// EpdCategorySet::Category
			case 13 : return 3;
			// EpdDataset::Type
			case 14 : return 4;
			// EpdDataset::Category
			case 15 : return 3;
			// EpdModuleDataset::para_t
			case 16 : return 8;
			// EpdModuleDataset::Module
			case 17 : return 16;
			// Infiltration::para_t
			case 18 : return 1;
			// Infiltration::AirChangeType
			case 19 : return 1;
			// InterfaceHeatConduction::modelType_t
			case 20 : return 1;
			// InterfaceHeatConduction::OtherZoneType
			case 21 : return 2;
			// InterfaceHeatConduction::para_t
			case 22 : return 1;
			// InternalLoad::para_t
			case 23 : return 8;
			// InternalLoad::Category
			case 24 : return 3;
			// InternalLoad::PersonCountMethod
			case 25 : return 2;
			// InternalLoad::PowerMethod
			case 26 : return 1;
			// KeywordList::MyParameters
			case 27 : return 1;
			// LcaSettings::para_t
			case 28 : return 2;
			// LcaSettings::Module
			case 29 : return 16;
			// LcaSettings::LcaCategory
			case 30 : return 3;
			// LcaSettings::UsageType
			case 31 : return 2;
			// LcaSettings::CalculationMode
			case 32 : return 1;
			// LcaSettings::CertificationSytem
			case 33 : return 0;
			// LccSettings::para_t
			case 34 : return 5;
			// LccSettings::intPara_t
			case 35 : return 2;
			// Material::para_t
			case 36 : return 5;
			// Material::Category
			case 37 : return 12;
			// MaterialLayer::para_t
			case 38 : return 1;
			// Network::PipeModel
			case 39 : return 1;
			// Network::ModelType
			case 40 : return 1;
			// Network::NetworkType
			case 41 : return 1;
			// Network::para_t
			case 42 : return 6;
			// NetworkBuriedPipeProperties::SoilType
			case 43 : return 2;
			// NetworkBuriedPipeProperties::para_t
			case 44 : return 1;
			// NetworkComponent::ModelType
			case 45 : return 14;
			// NetworkComponent::para_t
			case 46 : return 25;
			// NetworkComponent::intPara_t
			case 47 : return 1;
			// NetworkComponent::ComponentCategory
			case 48 : return 3;
			// NetworkController::ModelType
			case 49 : return 1;
			// NetworkController::ControlledProperty
			case 50 : return 5;
			// NetworkController::ControllerType
			case 51 : return 3;
			// NetworkController::para_t
			case 52 : return 7;
			// NetworkController::References
			case 53 : return 1;
			// NetworkFluid::para_t
			case 54 : return 2;
			// NetworkHeatExchange::ModelType
			case 55 : return 10;
			// NetworkHeatExchange::para_t
			case 56 : return 10;
			// NetworkHeatExchange::BuildingType
			case 57 : return 4;
			// NetworkHeatExchange::AmbientTemperatureType
			case 58 : return 2;
			// NetworkNode::NodeType
			case 59 : return 2;
			// NetworkPipe::para_t
			case 60 : return 8;
			// OutputDefinition::timeType_t
			case 61 : return 2;
			// Outputs::flag_t
			case 62 : return 3;
			// Room::para_t
			case 63 : return 2;
			// SubSurfaceComponent::SubSurfaceComponentType
			case 64 : return 2;
			// SubSurfaceComponent::para_t
			case 65 : return 0;
			// SupplySystem::SupplyType
			case 66 : return 3;
			// SupplySystem::para_t
			case 67 : return 3;
			// SurfaceHeating::para_t
			case 68 : return 4;
			// SurfaceHeating::Type
			case 69 : return 1;
			// VentilationNatural::para_t
			case 70 : return 0;
			// Window::Method
			case 71 : return 2;
			// Window::para_t
			case 72 : return 3;
			// WindowDivider::para_t
			case 73 : return 0;
			// WindowFrame::para_t
			case 74 : return 0;
			// WindowGlazingSystem::modelType_t
			case 75 : return 0;
			// WindowGlazingSystem::para_t
			case 76 : return 0;
			// WindowGlazingSystem::splinePara_t
			case 77 : return 0;
			// ZoneControlNaturalVentilation::para_t
			case 78 : return 3;
			// ZoneControlShading::para_t
			case 79 : return 5;
			// ZoneControlShading::Category
			case 80 : return 1;
			// ZoneControlThermostat::para_t
			case 81 : return 1;
			// ZoneControlThermostat::ControlValue
			case 82 : return 1;
			// ZoneControlThermostat::ControllerType
			case 83 : return 1;
			// ZoneIdealHeatingCooling::para_t
			case 84 : return 1;
			// ZoneTemplate::SubTemplateType
			case 85 : return 10;
		} // switch
		throw IBK::Exception(IBK::FormatString("Invalid enumeration type '%1'.")
			.arg(enumtype), "[KeywordList::MaxIndex]");
	}

	const char * KeywordList::Keyword(const char * const enumtype, int t) {
		const char * const kw = theKeyword(enum2index(enumtype), t);
		if (std::string(kw) == INVALID_KEYWORD_INDEX_STRING) {
			throw IBK::Exception(IBK::FormatString("Cannot determine keyword for enumeration type '%1' and index '%2'.")
				.arg(enumtype).arg(t), "[KeywordList::Keyword]");
		}
		return kw;
	}

	bool KeywordList::KeywordExists(const char * const enumtype, const std::string & kw) {
		int typenum = enum2index(enumtype);
		int i = 0;
		int maxIndexInCategory = MaxIndex( enumtype ); 
		for ( ; i <= maxIndexInCategory; ++i ) {
			std::string keywords = allKeywords(typenum, i);
			if (keywords == INVALID_KEYWORD_INDEX_STRING)
				continue;
			std::stringstream strm(keywords);
			int j = 0;
			std::string kws;
			while (strm >> kws) {
				if (kws == kw) {
					return true; // keyword exists
				}
				++j;
			}
		}
		return false; // nothing found keyword doesn't exist.
	}

	int KeywordList::Enumeration(const char * const enumtype, const std::string & kw, bool * deprecated) {
		int typenum = enum2index(enumtype);
		int i = 0;
		int maxIndexInCategory = MaxIndex( enumtype ); 
		for ( ; i <= maxIndexInCategory; ++i ) {
			std::string keywords = allKeywords(typenum, i);
			if (keywords == INVALID_KEYWORD_INDEX_STRING)
				continue;
			std::stringstream strm(keywords);
			int j = 0;
			std::string kws;
			while (strm >> kws) {
				if (kws == kw) {
					if (deprecated != nullptr) {
						*deprecated = (j != 0);
					}
					return i;
				}
				++j;
			}
		} // for ( ; i < maxIndexInCategory; ++i ) {
		throw IBK::Exception(IBK::FormatString("Cannot determine enumeration value for "
			"enumeration type '%1' and keyword '%2'.")
			.arg(enumtype).arg(kw), "[KeywordList::Enumeration]");
	}

	bool KeywordList::CategoryExists(const char * const enumtype) {
		return enum2index(enumtype) != -1;
	}

	void KeywordList::setParameter(IBK::Parameter para[], const char * const enumtype, int n, const double &val) {
		para[n] = IBK::Parameter(Keyword(enumtype, n), val, Unit(enumtype, n));
	}

	void KeywordList::setIntPara(IBK::IntPara para[], const char * const enumtype, int n, const int &val) {
		para[n] = IBK::IntPara(Keyword(enumtype, n), val);
	}

} // namespace VICUS
