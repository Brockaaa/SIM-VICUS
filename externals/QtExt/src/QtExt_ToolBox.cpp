...
void ToolBox::setCurrentIndex(unsigned int index) {
	...
	if(m_enableOpenMultiplePages){								// TODO: add to SIM-VICUS
		if(m_pages[index]->m_widget->isVisible()){
			m_pages[index]->m_widget->setVisible(false);
			m_pages[index]->m_arrowIcon->setPixmap(m_arrowRight);
			m_pages[index]->m_label->setActive(false);
		} else {
